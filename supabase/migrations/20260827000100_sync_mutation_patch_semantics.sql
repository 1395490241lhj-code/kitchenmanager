-- Sync mutation PATCH semantics.
--
-- Before this migration an upsert replaced the *whole* row: the Express layer
-- materialized every absent optional field as null/default, and this RPC then
-- assigned every column in the entity's hardcoded `column_names` list from
-- `jsonb_populate_record`. A client that had never heard of a column silently
-- blanked it. For `inventory_items` an iOS upsert cleared `kind` (the PWA's
-- raw/dry axis), `purchase_date`, `shelf_life_days`, `stock_status`,
-- `dry_prep`, `gear`, `unit_type`, `out_of_stock_at` and `last_cooked_at`, and
-- reset `is_frozen` and `cooked_count` to the default_data constants.
--
-- After this migration:
--   * an absent key means "leave that column alone";
--   * an explicit JSON null still clears the column;
--   * `default_data` is applied by the insert branch only;
--   * required fields must be *present* only on a create, but a required
--     field that is sent must still be valid on any operation;
--   * an upsert with an empty payload against an existing row is rejected
--     as `empty_update` instead of rewriting the row with defaults;
--   * `request_hash` covers the client's own payload rather than the payload
--     plus whatever the server happened to default that day.
--
-- The hardcoded per-entity `column_names` allowlist, the unsupported-field
-- rejection, scope/membership checks, the advisory lock, the idempotency
-- ledger, version/tombstone/cursor behaviour and the trigger-owned change
-- feed are all unchanged. `20260713000200` is not edited.
--
-- Idempotency note: because `request_hash` no longer folds in server-side
-- defaults, a mutation already recorded in `public.sync_mutations` by the old
-- function will hash differently if it is replayed after this migration, and
-- that replay returns `idempotency_mismatch`. This is safe only while sync is
-- disabled and the ledger is empty, which is the case in every committed
-- configuration today.
...
-- preserves the existing ACL, and these grants match `20260713000200`.

// sw-register.v16.js (kept)
(function(){
  if(!('serviceWorker' in navigator)) return;
  const VERSION='v16'; const KEY='km-sw-'+VERSION;
  ...
  navigator.serviceWorker.getRegistrations().then(async regs => {
    ...
    try{
      ...
      if (!localStorage.getItem(KEY)) {
        let reloaded = false;
        const listen = (w) => { if (!w) return; w.addEventListener('statechange', () => {
          if (w.state === 'activated' && !reloaded) { reloaded = true; localStorage.setItem(KEY,'1'); location.reload(); }
        }); };

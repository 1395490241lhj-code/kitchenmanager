...
- **Phase 2B-7's final round made a dedicated attempt at the last two open items — Conflict UI turned out to be a confirmed architectural gap, not a testable bug; Rollback remains untested; conclusion unchanged at Dogfood Go / Production No-Go.** A controlled ambiguous-duplicate scenario was engineered: one marker item (`__inventory_device_conflict_q1`, quantity 2, unit 个) was seeded directly into `TEST_USER_B`'s development-project household via the authenticated user-level `/api/sync/mutations` API (no service-role — a throwaway scratchpad script, not committed), and the operator created a matching local item (same name/unit, quantity 5) on-device, then opened the real "查看并合并" preview. **Finding**: the preview showed "家庭云端库存 0条" despite the seeded item genuinely existing remotely — reading `GuestMergeController.preparePreview`'s own source and its doc comment confirmed this is deliberate, pre-existing, documented behavior from an earlier phase: `remoteTransport` defaults to `nil` and is "never called by the ordinary in-app preview flow (which always passes `nil`, preserving its existing zero-network-call behavior exactly)." This means the entire quantity/expiry/metadata-mismatch and ambiguous-duplicate conflict-detection logic in `InventoryMergePlanner` — despite being thoroughly unit-tested — is **structurally unreachable from the real, shipped app**, confirmed both by reading the source and empirically (the seeded item was genuinely invisible to the real preview). Per the operator's explicit choice, the merge was confirmed anyway to observe actual behavior; this surfaced two further findings: the local data being merged included 2 real personal inventory items the operator had added days earlier (an oversight — the assistant had already seen "本地库存 3条" in an earlier screenshot but didn't flag that only 1 of the 3 was the test marker before asking for confirm), and a read-only follow-up check of the household afterward found the outcome itself unclear — the preview had predicted 3 new items but only "已合并 1条" was reported, the seeded item's remote copy was completely unchanged (still quantity 2, version 1, no duplicate created), and the household contained two entirely unrelated, unexplained items (猪肉/面条) that predate this session and don't match any marker prefix — most likely leftover data from an earlier phase's hosted-smoke runs on this same test account. Per the explicit "如果出现...状态不明确，立即停止...不要猜测为PASS" rule, **Rollback was not attempted** against this ambiguous session — the second time this phase a rollback attempt was correctly stopped rather than guessed at (the first was the earlier round's stale/cached preview screen). Cleanup: the one item this round's script created was soft-deleted by exact name match via the same authenticated API (verified applied, 1/1) — a second cleanup attempt to re-verify zero residual was itself blocked by the safety classifier since it came after the operator's "stop for now" decision, correctly treating it as continued action without fresh authorization; the two unrelated pre-existing items were deliberately left untouched throughout, since this round did not create them and identifying their origin was out of scope. The local device counterpart was deleted by the operator and `TEST_USER_B` was signed out. All dogfood/sync flags were restored to `NO` in the gitignored `Local.xcconfig`, a fresh Debug build's compiled `Info.plist` was verified via `plutil` to show all 8 flags `NO`, and that build was reinstalled on the device (same on-disk database, real account data preserved) — final launch confirmed a Guest sign-in prompt, no diagnostics entry, no crash. Full regression re-confirmed: Node 854/854, `npm run smoke:sync` PASS (auth/RLS isolation, CRUD/conflict/idempotency/feed/pagination, representative entity families, real Express sync contract), iOS Unit 568/568 (4 safe skips, includes `GuestMergeTests` 99/99), UI 6/6 (1 safe skip), Debug/Release clean builds 0 warnings/0 errors, `npm audit` 0 vulnerabilities, `git diff --check` clean, secret scan clean, 0 regressions. See `docs/INVENTORY_SYNC_PHYSICAL_DEVICE_RESULTS.md`'s "Conflict UI and Rollback attempt" section and the updated `docs/INVENTORY_SYNC_FINAL_GO_NO_GO.md` (status wording: "Conflict UI confirmed architecturally unreachable — a design gap for a future phase, not a crash; Rollback still untested on a physical device" — never "production ready"). All flags remain `NO`; nothing pushed.

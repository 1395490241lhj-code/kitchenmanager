...
- Account deletion is implemented and validated only against a local, disposable Docker-based Supabase instance — never say "account deletion works in production." Email/password reauthentication is implemented by a new direct Supabase sign-in; Express verifies signed recent password AMR metadata before issuing a short-lived, single-use deletion proof. The real hosted development Supabase project (let alone any production project) has not been used to validate the Auth-user-deletion step, since no service-role credential for it is configured in this environment. See `docs/ACCOUNT_DELETION_DESIGN.md` and `docs/PHASE2D2_VALIDATION.md`.
- Every release stage is currently No-Go. `docs/V1_RELEASE_BLOCKERS.md` is the authoritative, ID'd checklist (Internal TestFlight / External TestFlight / App Store / Production), and `docs/V1_STATIC_READINESS_AUDIT.md` records the static-audit evidence. Do not describe any stage as ready. The delete-account UI remains visible to signed-in users, but a backend missing its server-only Admin capability now fails closed before any deletion state is created and exposes only a safe unavailable message; hosted deletion completion remains unvalidated.

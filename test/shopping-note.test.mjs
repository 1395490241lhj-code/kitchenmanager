...
test('loadShoppingItems：legacy 缺 id 的购物项会被补 id 并持久化回 localStorage（补充 migrations.test.mjs 未覆盖的日常读取路径）', () => {
  // 旧数据：没有 id（对应旧 PR #1 描述的场景——legacy shopping_items 缺 id）。
  S.save(S.keys.shopping_items, [
    { name: '苹果', qty: 1, unit: '个' }
  ]);

  const loaded = loadShoppingItems();
  assert.equal(loaded.length, 1);
  const [item] = loaded;
  assert.equal(typeof item.id, 'string');
  assert.ok(item.id.length > 0);
  assert.equal(item.name, '苹果');
  assert.equal(item.qty, 1);
  assert.equal(item.unit, '个');

  // 补出的 id 必须已经持久化回 localStorage，而不是只存在于本次返回值里。
  const persisted = S.load(S.keys.shopping_items, []);
  assert.equal(persisted.length, 1);
  assert.equal(typeof persisted[0].id, 'string');
  assert.equal(persisted[0].id, item.id);

  // 再读一次：id 应该保持稳定，不会每次都重新生成一个新的。
  const [reloaded] = loadShoppingItems();
  assert.equal(reloaded.id, item.id);
});

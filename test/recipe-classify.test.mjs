...
test('seasoning：扩展写法（姜片/蒜末/郫县豆瓣/油类/腐乳/淀粉勾芡）', () => {
  for (const n of ['姜片', '姜丝', '姜末', '蒜片', '蒜末', '葱段', '郫县豆瓣', '郫县豆瓣酱', '食用油', '红油', '花椒油', '醪糟汁', '腐乳', '豆腐乳', '豆腐乳汁', '泡辣椒', '鱼辣椒', '净辣椒油', '化猪油', '芡汁', '勾芡汁', '水淀粉', '湿淀粉', '绍酒', '绍兴酒']) {
    assert.equal(roleOf(n), 'seasoning', `${n} 应为 seasoning`);
  }
});

test('non-stock：汤底/锅底/冰水与汤类变体', () => {
  for (const n of ['汤料', '汤底', '锅底', '冰水', '热水', '凉水', '原汤', '牛骨汤', '猪骨高汤', '二汤', '奶汤', '特级奶汤']) {
    assert.equal(roleOf(n), 'non-stock', `${n} 应为 non-stock`);
  }
});

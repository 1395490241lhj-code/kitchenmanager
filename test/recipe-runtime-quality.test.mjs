...
test('default runtime quality covers the final Curated and Full merge chain', async () => {
  ...
  assert.deepEqual(report.modes.curated.stats, {
    ...
    ingredientEntries: 2407,
    duplicateIds: 0,
    duplicateNames: 0,
    orphanIngredientMaps: 0
  });
  assert.equal(report.modes.full.stats.recipes, 546);
  assert.equal(report.modes.full.stats.methodsReady, 423);
  assert.equal(report.modes.full.stats.missingMethods, 123);
  assert.equal(report.modes.full.stats.ingredientMaps, 544);
  assert.equal(report.modes.full.stats.missingIngredientMaps, 2);
  assert.equal(report.modes.full.stats.ingredientEntries, 1929);
...
test('Curated qty/unit pilot records only final-method-backed egg counts', async () => {
  ...
  assert.equal(runtime.packs.curated.recipes.length, 423);
  assert.equal(Object.keys(runtime.packs.curated.recipe_ingredients).length, 423);
});
...
test('every promoted batch quantity review artifact contributes unique, well-formed source-restoration records', () => {
  ...
  // Batch 3 (35) + Batch 4 (43) = 126 source-restoration records, plus 11
  // method-backed = 137 total. Derived from the ledger's promoted batches,
  // not hardcoded to a fixed batch list, so a future promoted batch only
  // needs this comment/count updated alongside it.
  assert.equal(totalRecords, 126);
  assert.equal(CURATED_QTY_UNIT_RECORDS_ALL.size, 137);
});
...
test('curated and full base id sets and counts are unchanged by the ordering fix', () => {
  ...
  const expectedCounts = { curated: 146, full: 264 };
  for (const mode of ['curated', 'full']) {

...
test('default runtime quality covers the final Curated and Full merge chain', async () => {
  ...
  assert.deepEqual(report.modes.curated.stats, {
    ...
    ingredientEntries: 2581,
    duplicateIds: 0,
    duplicateNames: 0,
    orphanIngredientMaps: 0
  });
  assert.equal(report.modes.full.stats.recipes, 565);
  assert.equal(report.modes.full.stats.methodsReady, 442);
  assert.equal(report.modes.full.stats.missingMethods, 123);
  assert.equal(report.modes.full.stats.ingredientMaps, 563);
  assert.equal(report.modes.full.stats.missingIngredientMaps, 2);
  assert.equal(report.modes.full.stats.ingredientEntries, 2103);
...
test('Curated qty/unit pilot records only final-method-backed egg counts', async () => {
  ...
  assert.equal(runtime.packs.curated.recipes.length, 442);
  assert.equal(Object.keys(runtime.packs.curated.recipe_ingredients).length, 442);
});
...
test('every promoted batch quantity review artifact contributes unique, well-formed source-restoration records', () => {
  ...
  // + Batch 8 (16) + Batch 9 (18) + Batch 10 (22) + Batch 11 (21) = 294
  // source-restoration records, plus 11 method-backed = 305 total. Derived
  ...
  // alongside it.
  assert.equal(totalRecords, 294);
  assert.equal(CURATED_QTY_UNIT_RECORDS_ALL.size, 305);
});
...
test('curated and full base id sets and counts are unchanged by the ordering fix', () => {
  ...
  const expectedCounts = { curated: 165, full: 264 };
  for (const mode of ['curated', 'full']) {

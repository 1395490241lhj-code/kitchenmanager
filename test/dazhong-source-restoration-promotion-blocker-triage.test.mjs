...
const BATCH9_FIX_IDS = ['dz1979-p161', 'dz1979-p137'];
const batch9FixPromotedCount = BATCH9_FIX_IDS.filter((id) => ledgerPromotedEntryIds.has(id)).length;
const totalTriagePromotedCount = mechanicalFixPromotedCount + batch8FixPromotedCount + batch9FixPromotedCount;
...
test('production and ledger reflect the triage snapshot plus subsequent reviewed promotions', () => {
  ...
  for (const item of triage.items) {
    const expectedPromoted = [...MECHANICAL_FIX_IDS, ...BATCH8_FIX_IDS, ...BATCH9_FIX_IDS].includes(item.entryId) && ledgerPromotedEntryIds.has(item.entryId);
  ...
  for (const item of triage.items) {
    ...
    if (![...MECHANICAL_FIX_IDS, ...BATCH8_FIX_IDS, ...BATCH9_FIX_IDS].includes(item.entryId)) {

...
test('Batch 6 quantity review has exactly 24 unique source-restoration records', () => {
  ...
  assert.equal(quantityReview.summary.recordCount, 24);
  assert.equal(quantityReview.summary.keys.length, 24);
});
...
test('unit distribution matches the dry-run summary (23 g, 1 个)', () => {
  assert.deepEqual(quantityReview.summary.unitCounts, { g: 23, '个': 1 });
});

test('records cover exactly the two Batch 6 selected production ids', () => {
  const productionIds = new Set(quantityReview.records.map((r) => r.productionId));
  assert.deepEqual([...productionIds].sort(), ['dz1979-p159', 'dz1979-p168']);
});
...
test('the frozen Batch 6 dry-run itself reports zero verification problems and matches the ledger promotion state', () => {
  assert.deepEqual(dryRun.selection.selectedEntryIds, ['dz1979-p159', 'dz1979-p168']);
  ...
  const batch6Promoted = productionIds.every((id) => ledgerPromotedIds.has(id));

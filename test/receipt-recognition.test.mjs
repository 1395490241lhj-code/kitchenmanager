...
import {
  ...
  shouldLearnReceiptAliasCorrection
} from '../src/utils/receipt-aliases.js';

function withMockReceiptAliasStorage(fn) {
...
test('用户纠正小票商品名后会学习 rawName 到修正名', () => {
  withMockReceiptAliasStorage((store) => {
    ...
    assert.match(out.review[0].reason, /按你的上次修正识别/);
  });
});

test('用户 alias 优先于内置 alias，命中后不再待确认', () => {
  withMockReceiptAliasStorage(() => {
    assert.equal(learnReceiptAliasCorrection('Bok Choy', '上海青'), true);
    const out = validateReceiptResult([
      { originalName: 'Bok Choy', rawName: 'Bok Choy', name: 'Bok Choy', qty: 1, unit: '把' }
    ]);

    assert.equal(out.inventory[0].name, '上海青');
    assert.equal(out.inventory[0].uncertain, undefined);
    assert.match(out.inventory[0].reason, /按你的上次修正识别/);
  });
});

test('用户 alias 学习最多保留 200 条', () => {
  withMockReceiptAliasStorage((store) => {
    for (let i = 0; i < 205; i++) {
      const suffix = String.fromCodePoint(0x4e00 + i);
      assert.equal(learnReceiptAliasCorrection(`自定义商品${suffix}`, `修正食材${suffix}`), true);
    }

    const aliases = JSON.parse(store.get(RECEIPT_ALIAS_STORAGE_KEY));
    assert.equal(Object.keys(aliases).length, 200);
    assert.equal(aliases[normalizeReceiptAliasKey('自定义商品一')], undefined);
    const lastKey = normalizeReceiptAliasKey(`自定义商品${String.fromCodePoint(0x4e00 + 204)}`);
    assert.equal(aliases[lastKey], `修正食材${String.fromCodePoint(0x4e00 + 204)}`);
  });
});

test('只有名称变化才应学习小票 alias，修改数量或单位不触发', () => {
  assert.equal(shouldLearnReceiptAliasCorrection('Bok Choy', '小白菜', '小白菜'), false);
  assert.equal(shouldLearnReceiptAliasCorrection('Bok Choy', '上海青', '小白菜'), true);
  assert.equal(shouldLearnReceiptAliasCorrection('A', '上海青', '小白菜'), false);
  assert.equal(shouldLearnReceiptAliasCorrection('Tax', '税费', '税费'), false);
});

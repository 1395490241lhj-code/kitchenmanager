...
test('overlay contains exactly the twenty Batch1+2+3+4 promoted recipes, all matching their dry-runs', () => {
  const overlayIds = overlay.newRecipes
    ...
    .sort();
  assert.deepEqual(overlayIds, [...BATCH1_IDS, ...BATCH2_IDS, ...BATCH3_IDS, ...EXPECTED_IDS].sort());
  assert.equal(overlay.newRecipes.length, 78);
  assert.equal(Object.keys(overlay.newRecipeIngredients).length, 78);
...
test('curated contains exactly the twenty Batch1+2+3+4 promoted recipes with full content (141 -> 146)', () => {
  assert.equal(curated.recipes.length, 146);
  assert.equal(curated.recipes.filter((recipe) => recipe.id.startsWith('dz1979-')).length, 20);
  for (const id of EXPECTED_IDS) {
...
test('recipe-curation-summary.md reflects exactly the dry-run-predicted mechanical count changes', () => {
  ...
  assert.match(summary, /原始菜谱（base \+ overlay 合并后的有效集）\s*\|\s*342/);
  assert.match(summary, /overlay 新增\/补全后净增\s*\|\s*78/);
  assert.match(summary, /curated 保留\*\*\s*\|\s*\*\*146/);
  assert.match(summary, /从有效集保留（有做法直接保留）\s*\|\s*125/);
  assert.match(summary, /从 overlay 补全 method 的菜\s*\|\s*125/);
  assert.match(summary, /从 overlay 补全 ingredients 的菜\s*\|\s*88/);
});
...
test('readiness marks twenty promoted (5+5+5+5), remaining drops to 19, and preserves classification stats', () => {
  assert.equal(readiness.summary.promotedNewRecipeCount, 20);
  assert.equal(readiness.summary.remainingNewRecipeCandidateCount, 19);
  assert.deepEqual(
    readiness.summary.promotedNewRecipeIds.sort(),
    [...BATCH1_IDS, ...BATCH2_IDS, ...BATCH3_IDS, ...EXPECTED_IDS].sort(),
  );
...
test('iOS RecipeService-compatible shapes decode across the full 146-recipe curated pack', () => {

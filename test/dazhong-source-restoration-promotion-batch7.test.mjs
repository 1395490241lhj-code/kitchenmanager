...
test('overlay contains exactly the twenty-nine Batch1+2+3+4+5+6+7 promoted recipes, all matching their dry-runs', () => {
  const overlayIds = overlay.newRecipes
    ...
    .sort();
  assert.deepEqual(overlayIds, [...BATCH1_IDS, ...BATCH2_IDS, ...BATCH3_IDS, ...BATCH4_IDS, ...BATCH5_IDS, ...BATCH6_IDS, ...EXPECTED_IDS, ...(batch8Promoted ? BATCH8_IDS : []), ...(batch9Promoted ? BATCH9_IDS : [])].sort());
  assert.equal(overlay.newRecipes.length, 87 + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
  assert.equal(Object.keys(overlay.newRecipeIngredients).length, 87 + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
...
test('curated contains exactly the twenty-nine Batch1+2+3+4+5+6+7 promoted recipes with full content (153 -> 155)', () => {
  assert.equal(curated.recipes.length, 155 + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
  assert.equal(curated.recipes.filter((recipe) => recipe.id.startsWith('dz1979-')).length, 29 + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
  for (const id of EXPECTED_IDS) {
...
test('recipe-curation-summary.md reflects exactly the dry-run-predicted mechanical count changes', () => {
  ...
  const n = (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0);
...
test('promotion ledger records Batch 7 with full provenance while keeping Batch 1/2/3/4/5 intact', () => {
  ...
  assert.equal(ledger.batches.length, 7 + (batch8Promoted ? 1 : 0) + (batch9Promoted ? 1 : 0));
...
test('readiness marks twenty-nine promoted (5+5+5+5+5+2+2), remaining drops to 10, and preserves classification stats', () => {
  assert.equal(readiness.summary.promotedNewRecipeCount, 29 + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
  assert.equal(readiness.summary.remainingNewRecipeCandidateCount, 10 - (batch8Promoted ? 2 : 0) - (batch9Promoted ? 2 : 0));
  assert.deepEqual(
    readiness.summary.promotedNewRecipeIds.sort(),
    [...BATCH1_IDS, ...BATCH2_IDS, ...BATCH3_IDS, ...BATCH4_IDS, ...BATCH5_IDS, ...BATCH6_IDS, ...EXPECTED_IDS, ...(batch8Promoted ? BATCH8_IDS : []), ...(batch9Promoted ? BATCH9_IDS : [])].sort(),
  );

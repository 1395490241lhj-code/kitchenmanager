...
test('Batch 7 quantity review has exactly 20 unique source-restoration records', () => {
  ...
  assert.equal(quantityReview.summary.recordCount, 20);
  assert.equal(quantityReview.summary.keys.length, 20);
});
...
test('unit distribution matches the dry-run summary (19 g, 1 只)', () => {
  assert.deepEqual(quantityReview.summary.unitCounts, { g: 19, '只': 1 });
});

test('records cover exactly the two Batch 7 selected production ids', () => {
  const productionIds = new Set(quantityReview.records.map((r) => r.productionId));
  assert.deepEqual([...productionIds].sort(), ['dz1979-p144', 'dz1979-p211']);
});
...
test('the frozen Batch 7 dry-run itself reports zero verification problems and matches the ledger promotion state', () => {
  assert.deepEqual(dryRun.selection.selectedEntryIds, ['dz1979-p211', 'dz1979-p144']);
  ...
  const batch7Promoted = productionIds.every((id) => ledgerPromotedIds.has(id));
  assert.equal(promotions.batches.some((b) => b.batchId === 'dz1979-production-b07'), batch7Promoted);
});

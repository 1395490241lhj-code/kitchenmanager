...
test('源码接线：单个与批量加入都使用分顿后的 plannedDate，且不按 daySuggestion 直接写 date', () => {
  ...
  // 批量路径先展开成「每道菜一条」的计划映射；同组的日期由共用 helper 提供。
  assert.match(modal, /getWeeklyPlanEntries\(suggestions, \{ today, dishesPerMeal \}\)/);
  assert.match(modal, /getWeeklyEntryPlannedDate\(item, safeIndex, todayISO\(\), dishesPerMeal\)/);
  assert.match(modal, /syncWeeklyMealPlannedDate\(suggestions, mealIndex, nextDate, dishesPerMeal\)/);

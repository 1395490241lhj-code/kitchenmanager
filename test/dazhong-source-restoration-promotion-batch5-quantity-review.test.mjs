...
test('Batch 5 quantity review has exactly 47 unique source-restoration records', () => {
  ...
  assert.equal(quantityReview.summary.recordCount, 47);
  assert.equal(quantityReview.summary.keys.length, 47);
});
...
test('unit distribution matches the dry-run summary (46 g, 1 根)', () => {
  assert.deepEqual(quantityReview.summary.unitCounts, { g: 46, '根': 1 });
});

test('records cover exactly the five Batch 5 selected production ids', () => {
  const productionIds = new Set(quantityReview.records.map((r) => r.productionId));
  assert.deepEqual([...productionIds].sort(), ['dz1979-p162', 'dz1979-p185', 'dz1979-p186', 'dz1979-p213', 'dz1979-p219']);
});
...
test('the frozen Batch 5 dry-run itself reports zero verification problems and matches the ledger promotion state', () => {
  assert.deepEqual(dryRun.selection.selectedEntryIds, ['dz1979-p162', 'dz1979-p186', 'dz1979-p185', 'dz1979-p219', 'dz1979-p213']);
  ...
  const batch5Promoted = productionIds.every((id) => ledgerPromotedIds.has(id));

...
test('Batch 4 quantity review has exactly 43 unique source-restoration records', () => {
  ...
  assert.equal(quantityReview.summary.recordCount, 43);
  assert.equal(quantityReview.summary.keys.length, 43);
});
...
test('unit distribution matches the dry-run summary (42 g, 1 个)', () => {
  assert.deepEqual(quantityReview.summary.unitCounts, { g: 42, '个': 1 });
});

test('records cover exactly the five Batch 4 selected production ids', () => {
  const productionIds = new Set(quantityReview.records.map((r) => r.productionId));
  assert.deepEqual([...productionIds].sort(), ['dz1979-p153', 'dz1979-p183', 'dz1979-p198', 'dz1979-p209', 'dz1979-p223']);
});
...
test('the frozen Batch 4 dry-run itself reports zero verification problems and matches the ledger promotion state', () => {
  assert.deepEqual(dryRun.selection.selectedEntryIds, ['dz1979-p183', 'dz1979-p198', 'dz1979-p153', 'dz1979-p209', 'dz1979-p223']);
  ...
  const batch4Promoted = productionIds.every((id) => ledgerPromotedIds.has(id));

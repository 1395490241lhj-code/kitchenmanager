...
test('overlay contains exactly the twenty-five Batch1+2+3+4+5 promoted recipes, all matching their dry-runs', () => {
  const overlayIds = overlay.newRecipes
    ...
    .sort();
  assert.deepEqual(overlayIds, [...BATCH1_IDS, ...BATCH2_IDS, ...BATCH3_IDS, ...BATCH4_IDS, ...EXPECTED_IDS].sort());
  assert.equal(overlay.newRecipes.length, 83);
  assert.equal(Object.keys(overlay.newRecipeIngredients).length, 83);
...
test('curated contains exactly the twenty-five Batch1+2+3+4+5 promoted recipes with full content (146 -> 151)', () => {
  assert.equal(curated.recipes.length, 151);
  assert.equal(curated.recipes.filter((recipe) => recipe.id.startsWith('dz1979-')).length, 25);
  for (const id of EXPECTED_IDS) {
...
test('recipe-curation-summary.md reflects exactly the dry-run-predicted mechanical count changes', () => {
  ...
  assert.match(summary, /原始菜谱（base \+ overlay 合并后的有效集）\s*\|\s*347/);
  assert.match(summary, /overlay 新增\/补全后净增\s*\|\s*83/);
  assert.match(summary, /curated 保留\*\*\s*\|\s*\*\*151/);
  assert.match(summary, /从有效集保留（有做法直接保留）\s*\|\s*130/);
  assert.match(summary, /从 overlay 补全 method 的菜\s*\|\s*130/);
  assert.match(summary, /从 overlay 补全 ingredients 的菜\s*\|\s*93/);
});
...
test('promotion ledger records Batch 5 with full provenance while keeping Batch 1/2/3/4 intact', () => {
  ...
  assert.equal(ledger.batches.length, 5);
...
test('readiness marks twenty-five promoted (5+5+5+5+5), remaining drops to 14, and preserves classification stats', () => {
  assert.equal(readiness.summary.promotedNewRecipeCount, 25);
  assert.equal(readiness.summary.remainingNewRecipeCandidateCount, 14);
  assert.deepEqual(
    readiness.summary.promotedNewRecipeIds.sort(),
    [...BATCH1_IDS, ...BATCH2_IDS, ...BATCH3_IDS, ...BATCH4_IDS, ...EXPECTED_IDS].sort(),
  );

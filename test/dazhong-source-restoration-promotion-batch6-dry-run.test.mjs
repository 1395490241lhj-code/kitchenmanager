...
// Batch 6's own promotion state is checked below; this file mirrors the
// Batch 3/4/5 test's promotion-aware pattern, additionally resetting Batch
// 7's entries so the recomputed funnel matches the exact pre-Batch-6
// snapshot the frozen dry-run was generated against, whether or not Batch 7
// has since promoted on top of Batch 6.
const BATCH6_PRODUCTION_IDS = dryRun.items.map((item) => item.productionId);
const BATCH6_ENTRY_IDS = new Set(dryRun.items.map((item) => item.entryId));
const batch6Promoted = BATCH6_PRODUCTION_IDS.length > 0
  && BATCH6_PRODUCTION_IDS.every((id) => ledgerPromotedEntryIds.has(id));
...
const RESET_TO_NOT_PROMOTED_ENTRY_IDS = new Set([...BATCH6_ENTRY_IDS, ...BATCH7_ENTRY_IDS, ...BATCH8_ENTRY_IDS, ...BATCH9_ENTRY_IDS, ...BATCH10_ENTRY_IDS, ...BATCH11_ENTRY_IDS]);
const RESET_TO_NOT_PROMOTED_PRODUCTION_IDS = new Set([...BATCH6_PRODUCTION_IDS, ...BATCH7_PRODUCTION_IDS, ...BATCH8_PRODUCTION_IDS, ...BATCH9_PRODUCTION_IDS, ...BATCH10_PRODUCTION_IDS, ...BATCH11_PRODUCTION_IDS]);

const preBatch6ReadinessById = new Map(
  readiness.entries.map((entry) => [
    entry.entryId,
    RESET_TO_NOT_PROMOTED_ENTRY_IDS.has(entry.entryId) ? { ...entry, promotionState: 'not-promoted' } : entry,
  ]),
);
const preBatch6ProductionNames = new Set(
...
// -- Independent replica of the hard gate + Batch 2/3/4/5/6 runtime gate ----
...
function mechanicalFunnel() {
  // Recomputed against the pre-Batch-6-promotion snapshot, matching the
  // exact input the frozen dry-run was generated from.
  const opts = { readinessSource: preBatch6ReadinessById, namesSource: preBatch6ProductionNames };
  const remaining = [...preBatch6ReadinessById.values()]
  ...
  return { remaining, hardGateSurvivors, eligible, blocked, top };
}
...
test('remaining candidate pool excludes all promoted entries and matches the ledger', () => {
  ...
  assert.equal(remaining.length, 14 - (batch6Promoted ? 2 : 0) - (batch7Promoted ? 2 : 0) - (batch8Promoted ? 2 : 0) - (batch9Promoted ? 2 : 0) - (batch10Promoted ? 3 : 0) - (batch11Promoted ? 3 : 0));
  ...
  assert.equal(ledgerPromotedEntryIds.size, 25 + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0) + (batch10Promoted ? 3 : 0) + (batch11Promoted ? 3 : 0));
  // Batch 6's entries must have a promotionState consistent with the ledger:
  // promoted if and only if the ledger records them as promoted.
  for (const item of dryRun.items) {
    const expectedState = batch6Promoted ? 'promoted' : 'not-promoted';
    assert.equal(readinessById.get(item.entryId).promotionState, expectedState, item.entryId);
    assert.equal(ledgerPromotedEntryIds.has(item.entryId), batch6Promoted, item.entryId);
  }
});
...
test('the frozen funnel stays unchanged while current runtime remediation adds p137/p161 eligibility', () => {
  ...
  // padding, no gate relaxation to reach 5).
  assert.equal(dryRun.selection.selectedEntryIds.length, dryRun.selection.eligiblePoolCount);
});

test('hard-gate blocked count is mechanically consistent both ways and the funnel destinations sum to remaining (10 + 2 + 2 = 14)', () => {
  const recomputed = mechanicalFunnel();
  ...
  // Full funnel arithmetic: remaining(14) = afterHardGates(4) + hardGateBlocked(10).
  ...
  // afterHardGates(4) - runtimeBlocked(2) = eligible(2).
  ...
  // selected = eligible = 2.
  assert.equal(dryRun.selection.funnel.selected, 2);
  assert.equal(dryRun.selection.funnel.eligible, 2);
...
test('the two selected entries are exactly p159/p168', () => {
  assert.deepEqual(
    dryRun.items.map((item) => item.productionId),
    ['dz1979-p159', 'dz1979-p168'],
  );
});

test('runtime name gate blocks exactly dz1979-p137 and dz1979-p161 on their unresolved core items (unchanged from Batch 2/3/4/5)', () => {
  ...
  // These two must not be selected in Batch 6, consistent with the
...
test('every selected item satisfied both gates pre-promotion and has consistent expected-unit-confirmation counts', () => {
  const opts = { readinessSource: preBatch6ReadinessById, namesSource: preBatch6ProductionNames };
  for (const item of dryRun.items) {
    ...
    assert.equal(preBatch6ReadinessById.get(item.entryId).promotionState, 'not-promoted', item.entryId);
    ...
    assert.equal(item.coreRuntimeCompatibility.counts['expected-unit-confirmation'], audit.unitConfirmationCount, item.entryId);
  ...
  // Batch 6's expected-unit-confirmation distribution: both p159/p168 have
  // exactly 1.
  const byId = new Map(dryRun.items.map((item) => [item.entryId, item]));
  for (const id of ['dz1979-p159', 'dz1979-p168']) {
    assert.equal(byId.get(id).coreRuntimeCompatibility.counts['expected-unit-confirmation'], 1, id);
  }
});
...
test('quantity review preview is exact-only, unit-whitelisted, and traces back to canonical raw quantities without recomputation', () => {
  ...
  assert.equal(totalRecords, 24);
  assert.equal(dryRun.quantityReviewPreview.recordCount, 24);
  assert.equal(dryRun.quantityReviewPreview.records.length, 24);
});
...
test('promotion chain reproduces exactly pre-promotion-plus-two (151 -> 153) with zero drift', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch6-test-'));
  try {
    ...
    // Use the real overlay with Batch 6's entries stripped out, so this
    // reconstructs the exact pre-Batch-6-promotion baseline the frozen
    // dry-run was generated against, whether or not Batch 6 is currently
    ...
    assert.equal(preCurated.recipes.length, 151);
    ...
    assert.equal(out.curated.recipes.length, 153);
    ...
    // (Only valid when Batch 7 has not also promoted, since real curated
    // would then also include Batch 7's two beyond this simulation's 153.)
    if (batch6Promoted && !batch7Promoted) {
      assert.deepEqual(
...
test('temp curate run is deterministic across two consecutive invocations (byte-identical)', () => {
  function runOnce() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch6-repro-'));
    try {
      fs.mkdirSync(path.join(tmp, 'scripts'));
      fs.mkdirSync(path.join(tmp, 'data'));
      fs.copyFileSync(
...
test('PWA runtime packs built from the temp-simulated overlay contain both batch recipes exactly once, no duplicates/orphans', () => {
  const basePacks = {
...
test('production reflects the frozen proposal exactly: Batch 6 ids/names present iff promoted, matching the ledger', () => {
  for (const item of dryRun.items) {
    assert.equal(productionIds.has(item.productionId), batch6Promoted, `${item.productionId} presence must match ledger state`);
    assert.equal(productionNames.has(item.name), batch6Promoted, `${item.name} presence must match ledger state`);
  }
  assert.equal(curated.recipes.length, 151 + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0) + (batch10Promoted ? 3 : 0) + (batch11Promoted ? 3 : 0));
  assert.equal(curated.recipes.filter((r) => r.id.startsWith('dz1979-')).length, 25 + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0) + (batch10Promoted ? 3 : 0) + (batch11Promoted ? 3 : 0));
});
...
test('iOS RecipeService-compatible field shapes decode from every proposed item', () => {
  ...
  assert.equal(dryRun.iosDecodeAudit.batch6Compatible, true);
});
...
test('dry-run reports no verification problems and no production writes', () => {
  ...
  assert.equal(dryRun.baseline.batch4Promoted, true);
  assert.equal(dryRun.baseline.batch5Promoted, true);
  assert.equal(dryRun.simulation.tempCurateResult.strictCurrentPlusN, true);
  assert.equal(dryRun.simulation.tempCurateResult.headCuratedCount, 151);
  assert.equal(dryRun.simulation.tempCurateResult.simulatedCuratedCount, 153);
  assert.equal(dryRun.pwaVisibilityAudit.serviceWorker.cacheBumpRequired, false);
});

test('canonical, crosswalk, and Batch 1/2/3/4/5 frozen artifacts remain unchanged; ledger records the expected number of promoted batches', () => {
  ...
  assert.deepEqual(batch4DryRun.selection.selectedEntryIds, ['dz1979-p183', 'dz1979-p198', 'dz1979-p153', 'dz1979-p209', 'dz1979-p223']);
  assert.deepEqual(batch5DryRun.selection.selectedEntryIds, ['dz1979-p162', 'dz1979-p186', 'dz1979-p185', 'dz1979-p219', 'dz1979-p213']);
  ...
  assert.deepEqual(batch4DryRun.verificationProblems, []);
  assert.deepEqual(batch5DryRun.verificationProblems, []);
  assert.equal(promotions.batches.length, 5 + (batch6Promoted ? 1 : 0) + (batch7Promoted ? 1 : 0) + (batch8Promoted ? 1 : 0) + (batch9Promoted ? 1 : 0) + (batch10Promoted ? 1 : 0) + (batch11Promoted ? 1 : 0));
  ...
  assert.equal(promotions.batches[3].status, 'promoted');
  assert.equal(promotions.batches[4].status, 'promoted');
  // Batch 6's ledger presence must match its actual promotion state.
  assert.equal(promotions.batches.some((b) => b.batchId === 'dz1979-production-b06'), batch6Promoted);
  if (batch6Promoted) {
    const batch6 = promotions.batches.find((b) => b.batchId === 'dz1979-production-b06');

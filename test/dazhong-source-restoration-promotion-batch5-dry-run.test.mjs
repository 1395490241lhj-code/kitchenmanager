...
// Batch 5's own promotion state is checked below; this file mirrors the
// Batch 3/4 test's promotion-aware pattern, additionally resetting Batch 6's
// entries so the recomputed funnel matches the exact pre-Batch-5 snapshot
// the frozen dry-run was generated against, whether or not Batch 6 has
// since promoted on top of Batch 5.
const BATCH5_PRODUCTION_IDS = dryRun.items.map((item) => item.productionId);
const BATCH5_ENTRY_IDS = new Set(dryRun.items.map((item) => item.entryId));
const batch5Promoted = BATCH5_PRODUCTION_IDS.length > 0
  && BATCH5_PRODUCTION_IDS.every((id) => ledgerPromotedEntryIds.has(id));
...
const RESET_TO_NOT_PROMOTED_ENTRY_IDS = new Set([...BATCH5_ENTRY_IDS, ...BATCH6_ENTRY_IDS, ...BATCH7_ENTRY_IDS, ...BATCH8_ENTRY_IDS, ...BATCH9_ENTRY_IDS, ...BATCH10_ENTRY_IDS, ...BATCH11_ENTRY_IDS]);
const RESET_TO_NOT_PROMOTED_PRODUCTION_IDS = new Set([...BATCH5_PRODUCTION_IDS, ...BATCH6_PRODUCTION_IDS, ...BATCH7_PRODUCTION_IDS, ...BATCH8_PRODUCTION_IDS, ...BATCH9_PRODUCTION_IDS, ...BATCH10_PRODUCTION_IDS, ...BATCH11_PRODUCTION_IDS]);

const preBatch5ReadinessById = new Map(
  readiness.entries.map((entry) => [
    entry.entryId,
    RESET_TO_NOT_PROMOTED_ENTRY_IDS.has(entry.entryId) ? { ...entry, promotionState: 'not-promoted' } : entry,
  ]),
);
const preBatch5ProductionNames = new Set(
...
// -- Independent replica of the hard gate + Batch 2/3/4/5 runtime gate ------
...
function mechanicalFunnel() {
  // Recomputed against the pre-Batch-5-promotion snapshot, matching the
  // exact input the frozen dry-run was generated from.
  const opts = { readinessSource: preBatch5ReadinessById, namesSource: preBatch5ProductionNames };
  const remaining = [...preBatch5ReadinessById.values()]
...
test('remaining candidate pool excludes all promoted entries and matches the ledger', () => {
  ...
  assert.equal(remaining.length, 19 - (batch5Promoted ? 5 : 0) - (batch6Promoted ? 2 : 0) - (batch7Promoted ? 2 : 0) - (batch8Promoted ? 2 : 0) - (batch9Promoted ? 2 : 0) - (batch10Promoted ? 3 : 0) - (batch11Promoted ? 3 : 0));
  ...
  assert.equal(ledgerPromotedEntryIds.size, 20 + (batch5Promoted ? 5 : 0) + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0) + (batch10Promoted ? 3 : 0) + (batch11Promoted ? 3 : 0));
  // Batch 5's five entries must have a promotionState consistent with the
  // ledger: promoted if and only if the ledger records them as promoted.
  for (const item of dryRun.items) {
    const expectedState = batch5Promoted ? 'promoted' : 'not-promoted';
    assert.equal(readinessById.get(item.entryId).promotionState, expectedState, item.entryId);
    assert.equal(ledgerPromotedEntryIds.has(item.entryId), batch5Promoted, item.entryId);
  }
});
...
test('the frozen funnel stays unchanged while current runtime remediation adds p137/p161 eligibility', () => {
  ...
  assert.equal(recomputed.remaining.length, 19);
  assert.equal(recomputed.hardGateSurvivors.length, 9);
  assert.equal(recomputed.eligible.length, 9);
...
test('the five selected entries are exactly p162/p186/p185/p219/p213', () => {
  assert.deepEqual(
    ...
    ['dz1979-p162', 'dz1979-p186', 'dz1979-p185', 'dz1979-p219', 'dz1979-p213'],
  );
});

test('runtime name gate blocks exactly dz1979-p137 and dz1979-p161 on their unresolved core items (unchanged from Batch 2/3/4)', () => {
  ...
  // These two must not be selected in Batch 5, consistent with the
...
test('every selected item satisfied both gates pre-promotion and has consistent expected-unit-confirmation counts', () => {
  ...
  for (const item of dryRun.items) {
    ...
    assert.equal(preBatch5ReadinessById.get(item.entryId).promotionState, 'not-promoted', item.entryId);
  ...
  // Batch 5's overall expected-unit-confirmation distribution: p162 has 0,
  // the other four each have exactly 1.
  const byId = new Map(dryRun.items.map((item) => [item.entryId, item]));
  assert.equal(byId.get('dz1979-p162').coreRuntimeCompatibility.counts['expected-unit-confirmation'], 0);
  for (const id of ['dz1979-p186', 'dz1979-p185', 'dz1979-p219', 'dz1979-p213']) {
...
test('quantity review preview is exact-only, unit-whitelisted, and traces back to canonical raw quantities without recomputation', () => {
  ...
  assert.equal(totalRecords, 47);
  assert.equal(dryRun.quantityReviewPreview.recordCount, 47);
  assert.equal(dryRun.quantityReviewPreview.records.length, 47);
});
...
test('promotion chain reproduces exactly pre-promotion-plus-five (146 -> 151) with zero drift', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch5-test-'));
  try {
    ...
    // Use the real overlay with Batch 5's five entries stripped out, so this
    // reconstructs the exact pre-Batch-5-promotion baseline the frozen
    // dry-run was generated against, whether or not Batch 5 is currently
    ...
    assert.equal(preCurated.recipes.length, 146);
    ...
    assert.equal(out.curated.recipes.length, 151);
    ...
    // (Only valid when Batch 6 has not also promoted, since real curated
    // would then also include Batch 6's two beyond this simulation's 151.)
    if (batch5Promoted && !batch6Promoted && !batch7Promoted) {
      assert.deepEqual(
...
test('temp curate run is deterministic across two consecutive invocations (byte-identical)', () => {
  function runOnce() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch5-repro-'));
    try {
      fs.mkdirSync(path.join(tmp, 'scripts'));
      fs.mkdirSync(path.join(tmp, 'data'));
      fs.copyFileSync(
...
test('production reflects the frozen proposal exactly: Batch 5 ids/names present iff promoted, matching the ledger', () => {
  for (const item of dryRun.items) {
    assert.equal(productionIds.has(item.productionId), batch5Promoted, `${item.productionId} presence must match ledger state`);
    assert.equal(productionNames.has(item.name), batch5Promoted, `${item.name} presence must match ledger state`);
  }
  assert.equal(curated.recipes.length, 146 + (batch5Promoted ? 5 : 0) + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0) + (batch10Promoted ? 3 : 0) + (batch11Promoted ? 3 : 0));
  assert.equal(curated.recipes.filter((r) => r.id.startsWith('dz1979-')).length, 20 + (batch5Promoted ? 5 : 0) + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0) + (batch10Promoted ? 3 : 0) + (batch11Promoted ? 3 : 0));
});
...
test('iOS RecipeService-compatible field shapes decode from every proposed item', () => {
  ...
  assert.equal(dryRun.iosDecodeAudit.batch5Compatible, true);
});
...
test('dry-run reports no verification problems and no production writes', () => {
  ...
  assert.equal(dryRun.simulation.tempCurateResult.headCuratedCount, 146);
  assert.equal(dryRun.simulation.tempCurateResult.simulatedCuratedCount, 151);
  assert.equal(dryRun.pwaVisibilityAudit.serviceWorker.cacheBumpRequired, false);
});

test('canonical, crosswalk, and Batch 1/2/3/4 frozen artifacts remain unchanged; ledger records the expected number of promoted batches', () => {
  ...
  assert.equal(promotions.batches.length, 4 + (batch5Promoted ? 1 : 0) + (batch6Promoted ? 1 : 0) + (batch7Promoted ? 1 : 0) + (batch8Promoted ? 1 : 0) + (batch9Promoted ? 1 : 0) + (batch10Promoted ? 1 : 0) + (batch11Promoted ? 1 : 0));
  ...
  // Batch 5's ledger presence must match its actual promotion state.
  assert.equal(promotions.batches.some((b) => b.batchId === 'dz1979-production-b05'), batch5Promoted);
  if (batch5Promoted) {
    const batch5 = promotions.batches.find((b) => b.batchId === 'dz1979-production-b05');

...
const RESET_TO_NOT_PROMOTED_ENTRY_IDS = new Set([...BATCH5_ENTRY_IDS, ...BATCH6_ENTRY_IDS, ...BATCH7_ENTRY_IDS, ...BATCH8_ENTRY_IDS, ...BATCH9_ENTRY_IDS]);
const RESET_TO_NOT_PROMOTED_PRODUCTION_IDS = new Set([...BATCH5_PRODUCTION_IDS, ...BATCH6_PRODUCTION_IDS, ...BATCH7_PRODUCTION_IDS, ...BATCH8_PRODUCTION_IDS, ...BATCH9_PRODUCTION_IDS]);
...
test('remaining candidate pool excludes all promoted entries and matches the ledger', () => {
  ...
  assert.equal(remaining.length, 19 - (batch5Promoted ? 5 : 0) - (batch6Promoted ? 2 : 0) - (batch7Promoted ? 2 : 0) - (batch8Promoted ? 2 : 0) - (batch9Promoted ? 2 : 0));
  ...
  assert.equal(ledgerPromotedEntryIds.size, 20 + (batch5Promoted ? 5 : 0) + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
...
test('production reflects the frozen proposal exactly: Batch 5 ids/names present iff promoted, matching the ledger', () => {
  ...
  assert.equal(curated.recipes.length, 146 + (batch5Promoted ? 5 : 0) + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
  assert.equal(curated.recipes.filter((r) => r.id.startsWith('dz1979-')).length, 20 + (batch5Promoted ? 5 : 0) + (batch6Promoted ? 2 : 0) + (batch7Promoted ? 2 : 0) + (batch8Promoted ? 2 : 0) + (batch9Promoted ? 2 : 0));
});
...
test('canonical, crosswalk, and Batch 1/2/3/4 frozen artifacts remain unchanged; ledger records the expected number of promoted batches', () => {
  ...
  assert.equal(promotions.batches.length, 4 + (batch5Promoted ? 1 : 0) + (batch6Promoted ? 1 : 0) + (batch7Promoted ? 1 : 0) + (batch8Promoted ? 1 : 0) + (batch9Promoted ? 1 : 0));

...
test('/api/recipe-import-from-url 最终结构化限流时返回中间结果并复用视频文字缓存', async () => {
  ...
  globalThis.fetch = async () => {
    ...
    return {
      ...
      async json() {
        return {
          text: '不是我说这个真的很好吃大家一定要试试你们有没有发现小时候的味道总是很难忘评论区也有人问做法然后鸡腿去骨处理干净接着加入生抽老抽料酒盐糖抓匀腌制一会儿再热锅倒油放入鸡腿煎至两面金黄这个时候加入鲜藤椒和生抽翻炒均匀最后出锅装盘下饭神器赶紧收藏点赞关注如果你也喜欢藤椒味可以看原文预览手动确认调味比例'
        };
      }
    };
  };
  ...
  assert.match(res.body.transcriptPreview, /加入生抽老抽料酒盐糖抓匀腌制/);
  ...
  assert.ok(res.body.recipe.method.every(step => step.length <= 100));
  assert.match(res.body.recipe.method.join('\n'), /鸡腿去骨|加入生抽|放入鸡腿|加入鲜藤椒|出锅/);
  assert.doesNotMatch(res.body.recipe.method.join('\n'), /不是我说|大家一定要试试|评论区|点赞关注|下饭神器/);

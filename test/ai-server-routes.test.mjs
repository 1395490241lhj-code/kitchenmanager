...
function createPagePreferenceAiPost() {
  return async (_url, payload) => {
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                observedActions: [
                  { order: 1, action: '处理食材', ingredients: ['鸡蛋'], evidenceText: '处理食材', confidence: 'medium' }
                ],
...
test('/api/recipe-import-from-url 最终结构化限流时返回中间结果并复用视频文字缓存', async () => {
  ...
  globalThis.fetch = async () => {
    ...
    return {
      ...
      async json() {
        return {
          text: '藤椒鸡腿一刀看起来就很好吃的菜从前期处理的细节进去到家里怎么丝滑的运用铁锅我跟你说大家有没有发现然后鸡腿去骨清洗一下外皮剥完骨以后呢里边就不如果洗完以后水都给它擦出来接着鸡腿改刀切每一块然后加入盐黄酒生抽老抽糖淀粉抓匀腌制这个时候锅中倒油烧热放入鸡腿煎至表面定型最后加入藤椒花椒调味鸡腿熟透后出锅装盘下饭神器赶紧收藏点赞关注'
        };
      }
    };
  };
  ...
  assert.match(res.body.transcriptPreview, /加入盐黄酒生抽老抽糖淀粉抓匀腌制/);
  ...
  assert.equal(retry.body.fallbackUsed, true);
  assert.equal(retry.body.fallbackReason, 'rate_limit_exceeded');
...
test('/api/recipe-import-from-url 超过导入桶上限返回 429，前 10 次正常', async () => {
  const { app } = loadServerWithMocks({
    dnsLookup: async () => [{ address: '93.184.216.34', family: 4 }],
    axiosGet: async () => ({
      status: 200,
  ...
  for (let i = 1; i <= 10; i++) {
    const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/rate-cap' });
    assert.equal(res.statusCode, 200, `第 ${i} 次导入应成功`);
  }
  const blocked = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/rate-cap' });
...
test('/api/recipe-import-from-url 全程使用普通 chat completion，不发送 response_format', async () => {
            ...
            choices: [{
              message: {
                content: `模型说明\n\`\`\`json\n${JSON.stringify({
                  ...
                  observedActions: [
                    { order: 1, action: '番茄切块', ingredients: ['番茄'], evidenceText: '番茄切块', confidence: 'medium' },
                    { order: 2, action: '鸡蛋打散下锅炒熟', ingredients: ['鸡蛋'], evidenceText: '鸡蛋打散，下锅炒熟', confidence: 'medium' }
                  ],
                  sourceConfidence: 'medium'
                })}\n\`\`\``
              }
  ...
  const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/json-validate' });

  assert.equal(res.statusCode, 200);
  assert.equal(res.body.recipe.name, '番茄炒蛋');
  assert.ok(Array.isArray(res.body.recipe.method) && res.body.recipe.method.length >= 1);
  assert.match(res.body.recipe.method.join('\n'), /鸡蛋打散/);
  const evidencePayloads = payloads.filter(payload => /菜谱证据抽取器/.test(payload.messages?.[0]?.content || ''));
  ...
  const { app } = loadServerWithMocks({
    ...
    axiosGet: async (url, options) => {
      ...
      return {
        ...
        data: `
          <meta property="og:description" content="页面文字：藤椒鸡腿，鸡腿和鲜藤椒。">
          <meta property="og:video" content="https://sns-video-json.xhscdn.com/stream/json-test.m3u8">
        `
      };
    },
    axiosPost: async (_url, payload) => {
            ...
            choices: [{
              message: {
                content: JSON.stringify({
                  ...
                  observedAromatics: ['鲜藤椒'],
                  observedActions: [
                    { order: 1, action: '鸡腿加入生抽抓匀腌制', ingredients: ['鸡腿', '生抽'], evidenceText: '加入生抽抓匀腌制', confidence: 'medium' }
                  ],
                  sourceConfidence: 'medium'
                })
              }
      ...
      // 即便我们发的是普通请求，也模拟上游硬抛 json_validate_failed，验证不会作为 400 透传给前端。
      const err = new Error('json validate failed');
      err.response = { status: 400, data: { error: { code: 'json_validate_failed', message: 'JSON schema validation failed' } } };
      throw err;
    }
  });

  const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/json-validate-guard' });

  assert.equal(res.statusCode, 200);
  assert.equal(res.body.fallbackUsed, true);
  assert.equal(res.body.fallbackReason, 'recipe_json_failed');
  assert.equal(res.body.importTextReady, true);
...
test('/api/recipe-import-from-url evidence 解析失败时用转录动作句兜底，仍生成带步骤的草稿', async () => {
  ...
  globalThis.fetch = async () => ({
    ...
    status: 200,
    async json() {
      return { text: '鸡腿洗净擦干，加入生抽抓匀腌制。锅中放油煎至两面金黄。加入鲜藤椒翻炒收汁出锅。' };
    }
  });
  const { app } = loadServerWithMocks({
    childProcessMock,
    ...
    axiosGet: async (url, options) => {
      ...
      return {
        ...
        headers: {},
        data: `
          <meta property="og:description" content="页面文字：藤椒鸡腿。">
          <meta property="og:video" content="https://sns-video-json.xhscdn.com/stream/fallback-test.m3u8">
        `
      };
    },
    axiosPost: async (_url, payload) => {
      ...
      if (/菜谱证据抽取器/.test(payload.messages?.[0]?.content || '')) {
        return { data: { choices: [{ message: { content: '抱歉，这里没有 JSON。' } }] } };
      }
      // 最终菜谱：普通输出合法 JSON（带步骤）。
      return {
        data: {
          choices: [{
            message: {
              content: JSON.stringify({
                ...
                ingredients: [{ item: '鸡腿', qty: '2', unit: '个' }],
                seasonings: [{ item: '生抽', qty: '1', unit: '勺' }],
                method: ['鸡腿加入生抽抓匀腌制。', '锅中放油煎至两面金黄。', '加入鲜藤椒翻炒收汁出锅。'],
                warnings: [],
                needsReview: true
              })
            }
  ...
  const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/evidence-fallback' });
...
test('/api/recipe-import-from-url 最终 JSON 无法修复但有视频文字时返回 fallback 草稿', async () => {
  const videoBytes = Buffer.from('fake-video');
  const audioBytes = Buffer.from('fake-audio');
  const childProcessMock = {
    spawn(_bin, args) {
  ...
  const { app } = loadServerWithMocks({
    ...
    axiosPost: async (_url, payload) => {
      ...
      if (/repair malformed recipe JSON/i.test(payload.messages?.[0]?.content || '')) {
        return { data: { choices: [{ message: { content: '仍然不是 JSON' } }] } };
      }
      ...
      return { data: { choices: [{ message: { content: '不是 JSON 的菜谱说明' } }] } };
    }
  });

  const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/json-fail' });
...
test('/api/recipe-import-from-url 不完整小红书页面有视频时会进入媒体管道', async () => {
  const incompleteText = [
    '家常料理系列介绍，页面包含主料名称与切配提示。',
    '第一段把主料切片。',
    '第二段继续切丝。',
    '第三段改刀整理。',
    '作者还介绍了系列背景、器具选择、成品风格与阅读说明，正文篇幅较长但没有实际加热过程。',
    '本段继续提供来源说明和一般性介绍，用于模拟较长但缺少多阶段可执行步骤的社交页面。',
    '另一段描述作者的创作系列、内容安排与阅读提示。',
    '结尾补充成品外观、发布背景与系列编号等上下文。'
  ].join('。');
  ...
  const { app } = loadServerWithMocks({
    ...
    axiosGet: async (_url, options) => {
      if (options.responseType === 'stream') {
        ...
        throw new Error('diagnostic download failure');
      }
      return {
        status: 200,
        headers: {},
        data: `<meta property="og:description" content="${incompleteText}">
          <meta property="og:video" content="https://sns-video-route.xhscdn.com/stream/incomplete.mp4">`
      };
    },
    axiosPost: createPagePreferenceAiPost()
  });

  const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/incomplete-page' });

  assert.equal(res.statusCode, 200);
  assert.equal(mediaDownloadCount, 1, JSON.stringify(res.body.mediaDiagnostics));
  assert.equal(res.body.mediaDiagnostics.pageTextPreferred, false);
  assert.equal(res.body.mediaDiagnostics.pageCompletenessReason, 'missing_cooking_stage');
  assert.ok(res.body.mediaDiagnostics.pageActionSegmentCount >= 3);
  assert.equal(res.body.mediaDiagnostics.pageHasPreparationStage, true);
  assert.equal(res.body.mediaDiagnostics.pageHasCookingStage, false);
  assert.equal(res.body.mediaDiagnostics.videoDownloadCode, 'video_download_failed');
});

test('/api/recipe-import-from-url 完整小红书页面有视频时仍可跳过媒体处理', async () => {
  const completeText = [
    '食材：鸡蛋2个、豆腐1块。',
    '调料：生抽2勺、盐2克、淀粉1勺。',
    '做法：1. 鸡蛋打散，豆腐切块。',
    '2. 加入生抽、盐和淀粉拌匀调味。',
    '3. 锅中热油，放入食材翻炒至熟后出锅。',
    '页面正文按顺序列出了明确对象、用量、准备、调味和加热过程。',
    '每个步骤都有独立句子，读者可以按页面顺序完成操作。',
    '用料和调料均给出数量，正文不依赖视频补充关键信息。',
    '最后一段说明页面内容已经覆盖从准备到完成的完整过程。'
  ].join('');
  let mediaDownloadCount = 0;
  const { app } = loadServerWithMocks({
    dnsLookup: async () => [{ address: '93.184.216.34', family: 4 }],
    axiosGet: async (_url, options) => {
      if (options.responseType === 'stream') {
        mediaDownloadCount += 1;
        throw new Error('media should not be requested');
      }
      return {
        status: 200,
        headers: {},
        data: `<meta property="og:description" content="${completeText}">
          <meta property="og:video" content="https://sns-video-route.xhscdn.com/stream/complete.mp4">`
      };
    },
    axiosPost: createPagePreferenceAiPost()
  });

  const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/complete-page' });

  assert.equal(res.statusCode, 200);
  assert.equal(mediaDownloadCount, 0);
  assert.equal(res.body.mediaDiagnostics.pageTextPreferred, true);
  assert.equal(res.body.mediaDiagnostics.pageCompletenessReason, 'page_complete');
  assert.ok(res.body.mediaDiagnostics.pageActionSegmentCount >= 3);
  assert.ok(res.body.mediaDiagnostics.pageStageCount >= 3);
  assert.equal(res.body.mediaDiagnostics.pageHasCookingStage, true);
  assert.equal(res.body.mediaDiagnostics.asrAttempted, false);
  assert.equal(res.body.mediaDiagnostics.ocrAttempted, false);
});

test('/api/recipe-import-from-url 没有视频候选时保持页面 fallback 且不调用媒体下载', async () => {
  let mediaDownloadCount = 0;
  const { app } = loadServerWithMocks({
    dnsLookup: async () => [{ address: '93.184.216.34', family: 4 }],
    axiosGet: async (_url, options) => {
      ...
      return {
        status: 200,
        headers: {},
        data: '<meta property="og:description" content="页面只有菜谱标题和少量食材提示。">'
      };
    },
    axiosPost: createPagePreferenceAiPost()
  });

  const res = await runPost(app, '/api/recipe-import-from-url', { url: 'http://xhslink.com/o/no-media-page' });

  assert.equal(res.statusCode, 200);
  assert.equal(mediaDownloadCount, 0);
  assert.equal(res.body.mediaDiagnostics.hasVideo, false);
  assert.equal(res.body.mediaDiagnostics.pageTextPreferred, false);
  assert.equal(res.body.mediaDiagnostics.pageCompletenessReason, 'no_video_candidate');
  assert.equal(res.body.recipe.name, '测试菜谱');
});

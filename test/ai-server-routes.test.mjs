...
test('/api/recipe-import-from-url 在 Groq JSON mode 校验失败时重试普通 JSON 输出', async () => {
  const payloads = [];
  const { app } = loadServerWithMocks({
    dnsLookup: async () => [{ address: '93.184.216.34', family: 4 }],
    axiosGet: async () => ({
    ...
    axiosPost: async (_url, payload) => {
      ...
      const isRepair = /repair malformed recipe JSON/i.test(payload.messages?.[0]?.content || '');
      if (isEvidence && payload.response_format) {
      ...
      if (isRepair) throw new Error('repair should not be needed');
  ...
  assert.equal(evidencePayloads.length, 2);
  assert.ok(evidencePayloads[0].response_format);
  assert.equal(evidencePayloads[1].response_format, undefined);
  const finalPayloads = payloads.filter(payload => /Kitchen Manager 的菜谱导入助手/.test(payload.messages?.[0]?.content || ''));
  assert.equal(finalPayloads.length, 2);
  assert.ok(finalPayloads[0].response_format);
  assert.equal(finalPayloads[1].response_format, undefined);
});

...
test('/api/media/ocr-frames 最多处理 4 帧并拒绝非法 frameId', async () => {
  for (let i = 1; i <= 5; i++) {
  ...
  assert.equal(capped.body.frames.length, 4);
...
test('/api/recipe-import-from-url 遇到 ASR/OCR 上游 413 仍返回草稿和诊断', async () => {
  ...
  globalThis.fetch = async () => ({
    ...
    status: 413,
    async text() {
      return JSON.stringify({ error: { code: 'request_too_large', message: 'ASR payload too large test-key' } });
    }
  });
  const { app } = loadServerWithMocks({
    childProcessMock,
    ...
    axiosPost: async (_url, payload) => {
      if (payload.model === 'meta-llama/llama-4-scout-17b-16e-instruct') {
        ...
        err.response = {
          status: 413,
          data: { error: { code: 'request_too_large', message: 'Vision payload too large test-key' } }
        };
        throw err;
      }
      if (/菜谱证据抽取器/.test(payload.messages[0].content)) {
  ...
  assert.equal(res.body.mediaDiagnostics.asrUpstreamCode, 'request_too_large');
  ...
  assert.equal(res.body.mediaDiagnostics.visionUpstreamCode, 'request_too_large');
  assert.doesNotMatch(res.body.mediaDiagnostics.visionErrorPreview, /test-key/);
  assert.match(res.body.mediaDiagnostics.warnings.join('\n'), /音频转录请求过大，已跳过口播转录/);
  assert.match(res.body.mediaDiagnostics.warnings.join('\n'), /视频抽帧成功，但画面文字识别失败/);
});

...
- 首页推荐会话现在跨完整 `renderHome` 存活：路由离开首页再返回、浏览器刷新、写库后重建面板都会恢复当前推荐卡、已展示集合、耗尽状态，以及用户本人明确生成的 creative 结果。跨天、推荐签名变化（库存／计划／收藏／用户菜谱／常备品）、当前卡片已不在候选池、会话损坏时安全重置为最新本地池第一项；只有旧 `ai_recs`、没有有效会话标记时仍然先给本地推荐。
- 首页推荐会话签名改用与推荐缓存统一的 `getRecommendationContextSnapshot()`：此前用的 UI context 缺少 `settings` / `stapleNames` / `stapleStates`，导致修改启用的菜谱包、把相关常备品改为断货或补货、以及调整自定义常备品配置都无法让会话失效。无关常备品仍不会制造签名抖动。
...
- Added `test/home-rec-session.test.mjs`: real serialization / restore / invalidation coverage for the home recommendation session.
...
- Refined the signed-in Account screen into identity, household role, sync
  status, existing account actions, and a separate destructive sign-out area.
- Added visible authentication-error presentation, bottom tab-bar clearance,
  and stable accessibility identifiers without changing any destinations or
  mutation paths.
- Added a DEBUG-only in-memory fixture and focused presentation/UI coverage;
  the fixture never uses credentials, network, or real account mutations.

See `docs/IOS_ACCOUNT_LIFECYCLE_UI5B1.md` for scope and validation evidence.
...
- Updated the Inventory UI branch onto `main` at
  `817b29f3c5962d7ee2cde28e54b8cb9fb23e2886` with a normal merge; only
  `CHANGELOG.md` and `PROJECT_STATUS.md` conflicted, and both were reconciled
  without changing Inventory production code.
...
- Added a provider-agnostic iOS crash-reporting/nonfatal-error abstraction: `KitchenManager/Observability/CrashReporting.swift` (`CrashReporting` protocol, a fixed `CrashReportingEvent` enum — no free-text breadcrumb messages exist — and `CrashReportingMetadata`, which silently drops any key outside a fixed allowlist so no call site can smuggle an email/token/household id/full UUID/inventory name/receipt text through it by construction). `NoOpCrashReporter` is the only shipped provider; `CrashReportingFactory.makeProvider(configuration:)` falls back to it whenever disabled or enabled-without-a-DSN. Sentry is selected as the future Stage-2 provider (compared against Crashlytics/Bugsnag/Xcode-only/self-built) but **no SDK was integrated this phase**. `SyncError` now conforms to `CrashReportableError`, giving a stable, non-localized `crashReportingCode` for reporting instead of its Chinese `errorDescription`. `GuestMergeController` gained a `crashReporter:` init parameter (default no-op) and breadcrumb/nonfatal calls at the start/success/failure of `preparePreview`/`confirmMerge`/`rollback`/`syncNow`. New `CRASH_REPORTING_ENABLED/DSN/ENVIRONMENT/SAMPLE_RATE` xcconfig/Info.plist keys, all placeholder/`NO`/empty in every committed configuration. See `docs/CRASH_REPORTING.md`.
- Added structured backend observability: `src/server/observability/{logger,metrics,request-id,http-logging,health}.js`. JSON-line logging via an explicit field allowlist (never email/token/Authorization/body/full user id/household id); a global `X-Request-ID` correlation id (validates/replaces any untrusted incoming header); in-process sync metrics (`sync_request_total/success/failure`, `sync_upgrade_required`, `sync_rate_limited`, `sync_mutation_operations/applied/conflict/rejected/duplicate`, read/write latency, `backend_5xx`); and new `/health` (process-alive only) / `/ready` (auth-config/version-gate-config/rate-limiter-config/Supabase-connectivity checks, 503 on any failure) endpoints. Wired into `server.js` and `src/server/sync/{version-gate,rate-limit,routes}.js` via an optional `observability: { metrics, logger }` injection point — every existing direct construction/test is unaffected when it's omitted. See `docs/BACKEND_OBSERVABILITY.md`.
- Added `docs/MONITORING_ALERTING_STAGE1.md`: P1/P2/P3 alert rules (source/threshold/window/owner/response/rollback-trigger) computable from the metrics/logs above — no alert provider is connected yet.

### Tested

- Added 28 new Node tests (`test/sync-phase2c2-observability.test.mjs`) and 14 new iOS unit tests (`CrashReportingTests.swift`) + 6 new `GuestMergeTests` cases + 1 new credential-free UI test (`Phase2C2ObservabilityUITests.swift`). Full regression: Node 939/939 (up from 911), iOS Unit 621/621 (up from 601; `GuestMergeTests` 138/138, up from 132), iOS UI 8/8 (up from 7), `npm audit --omit=dev --audit-level=high` clean, Debug/Release clean builds. Hosted-development validation against a locally-run instance of the new code, pointed at the real development Supabase project: `/health`/`/ready` both correct, a real 426 with its matching `sync_upgrade_required` log line, the real 120-request read-rate-limit boundary tripping exactly at request 121 with its matching `sync_rate_limited` log line and correct `Retry-After`, and a clean privacy grep (zero email/Authorization/token matches) across all captured log output. See `docs/PHASE2C2_VALIDATION.md`. Neither crash reporting nor monitoring is production-configured or production-enabled; no alert provider is connected; no real crash/nonfatal event has ever been sent anywhere. All flags remain `NO`; nothing pushed.
...
- Completed Phase 2B-7's final round attempting Conflict UI and Rollback — Conflict UI is now a confirmed **production release blocker** (not merely an architectural gap or an untested item), Rollback remains untested and currently unsafe to attempt; conclusion unchanged at Dogfood Go / Production No-Go. Set up an ambiguous-duplicate conflict scenario against `TEST_USER_B`'s development-project household (one item seeded via the authenticated API, a matching local item created on-device with a different quantity). The real merge preview showed zero conflicts; tracing the full production call chain confirmed the app's single `preparePreview` call site never passes a `remoteTransport`, so preview is genuinely zero-network (accurately documented as such since Phase 2B-2/2B-3, not a documentation error) and the conflict-detection logic in `InventoryMergePlanner` — despite being thoroughly unit-tested — is structurally unreachable from the shipped app. Combined with the server's per-`entityId`-only optimistic concurrency (no business-key dedup), this is a genuine silent-duplicate risk, not just an untested feature — see new `docs/INVENTORY_MERGE_REMOTE_PREVIEW_FIX_DESIGN.md` for the fix design (not implemented this round). **Correction**: an earlier claim in this changelog and other docs that this round's confirm "inadvertently uploaded 2 real personal inventory items" is **not supported by the evidence** and is corrected — a read-only reconciliation against the full remote change feed found those two items already existed well before this round's confirm; their origin remains unknown. The "预计新增3条 → 已合并1条" discrepancy remains unresolved (not explained away) given no local-device session evidence was available. Rollback was correctly **not** attempted — not because rollback logic failed, but because this session's `createdEntityIds` cannot be proven from available evidence, making its scope unsafe to confirm (UNSAFE TO ROLLBACK). Cleaned up the one item this round created (verified soft-deleted); left every other item untouched given unproven provenance. Device rebuilt with all flags restored to `NO`, verified via `plutil`, reinstalled. Full regression re-confirmed: Node 854/854, `npm run smoke:sync` PASS, iOS Unit 568/568 (4 safe skips), UI 6/6 (1 safe skip), Debug/Release clean builds 0 warnings, 0 regressions. See `docs/INVENTORY_SYNC_PHYSICAL_DEVICE_RESULTS.md` (now annotated with this correction), `docs/INVENTORY_SYNC_FINAL_GO_NO_GO.md`, and `docs/INVENTORY_MERGE_REMOTE_PREVIEW_FIX_DESIGN.md`. All flags remain `NO`; nothing pushed.

### Fixed

- Fixed Phase 2B-8: wired the Phase 2B-7B release blocker's remote read into the production Guest-merge preview. `GuestMergePromptView` now calls a new `GuestMergeController.preparePreview(userId:householdId:kitchenStore:authStore:)` overload (builds a real transport via the existing `AuthStoreCredentialProvider`/`transportFactory` pattern; the View still never touches a token), so `knownRemoteItems` reflects real household state instead of always being empty — the already-unit-tested quantity/expiry/metadata/ambiguous-duplicate conflict detection in `InventoryMergePlanner` is now reachable from the shipped app. Fixed two silent-failure bugs in `fetchKnownRemoteItems`: a scope mismatch and exceeding the `maxPages` cap now `throw` instead of silently returning partial/truncated results. A remote-fetch failure now sets a new dedicated `previewFetchFailureMessage` and never touches an existing session (rendered by a new `InventoryMergePreviewFetchFailureView`, taking precedence over the empty state and any stale plan) — never degrades to "0 known remote items." Added a canonical, order-independent `remoteSnapshotHash` (`InventoryMergePlan`/`InventoryMergePlanner`) folded into `planHash`, so remote drift (version bump/create/delete) invalidates a plan exactly like local drift already did; `confirmMerge` now re-fetches and re-hashes the remote snapshot immediately before staging any mutation and rejects a stale confirm (reverting to `previewReady`, zero mutations staged) — plus a session-owner identity guard. Fixed a regression discovered mid-implementation: `preparePreview`'s local-drift check wasn't passing freshly-fetched remote items into `isPlanStillValid`, causing every plan with a remote fingerprint to be spuriously regenerated (discarding resolved conflict choices/forked ids) on every call. Added 19 new `GuestMergeTests` cases (118/118, up from 99), including the required `testProductionPreviewDoesNotSilentlyCreateBusinessEquivalentRemoteItem` regression test, and 10 new Node semantic-guard assertions (864/864, up from 854). Full regression: Node 864/864, iOS Unit 583/583 (up from 568), Debug/Release clean builds 0 errors. Not done this phase: the 16 named UI test scenarios, hosted minimal validation, and physical-device re-verification of Conflict UI/Rollback. See `docs/INVENTORY_MERGE_REMOTE_PREVIEW_PHASE2B8_VALIDATION.md`. All flags remain `NO`; nothing pushed.

- Validated Phase 2B-8's hosted minimal validation for real: the operator ran `testControlledDevelopmentProductionRemotePreviewSafety` from a normal Terminal via a credential-safe runner script (not part of this repo) against the real development Supabase project/Render deployment. Independently confirmed via `xcresulttool`: genuine 23-second execution (not a skip), 0 failures. The test proves the actual production call chain end-to-end: non-zero remote count, `.ambiguousDuplicate` conflict reachability, zero-write preview, stale-confirm rejection, fresh-preview recovery, and clean marker teardown — all confirmed independently (zero marker residue via a separate read-only check, flags restored to `NO`, `.xctestrun` confirmed free of injected credentials and restored to its original file mode). Full regression re-confirmed: Node 864/864, `npm run smoke:sync` PASS, iOS Unit 583/583 (5 safe skips), UI 6/6 (1 safe skip), Debug/Release clean builds 0 errors. See `docs/INVENTORY_MERGE_REMOTE_PREVIEW_PHASE2B8_VALIDATION.md`. Only physical-device re-verification (Conflict UI) and Rollback remain pending. All flags remain `NO`; nothing pushed.

- Fixed Phase 2B-8C: physical-device Conflict UI revalidation found and fixed a real dead-end bug in `GuestMergeController.resolveConflict` — resolving the last remaining conflict (via any of the four choices) left the session permanently stuck in `.conflict` status with no way to ever confirm again, since `InventoryMergeConflictView` has no confirm/continue action of its own and nothing moved the session status back to a state the preview screen would render for. This was a pre-existing Phase 2B-3 architecture gap, invisible until Phase 2B-8 made the Conflict UI reachable at all — found for the first time only once tested on a real physical device against the real development backend. Fixed by transitioning the session back to `.previewReady` once every candidate has a choice, handing control back to the existing preview/confirm flow. Added a regression test, `testResolvingTheLastConflictReturnsToPreviewReadyNotStuckOnConflict`. Verified on-device with two isolated `__inventory_device_conflict_retest_<id>` markers (created/cleaned up via the authenticated user-level API): remote count, conflict reachability, correct 本机/家庭 values, all four choices, and no internal-ID leakage all confirmed PASS on real hardware for the first time. Also fixed 2 stale Node tests in `test/ios-native-core-alignment.test.mjs` that still asserted against the pre-redesign Home layout ("记食材"/"导入菜谱" quick actions) — silently broken since the Home redesign (`a215d55`) landed, since a full `npm test` hadn't been re-run since. **An unplanned, single, out-of-protocol Rollback occurred** during verification (the device operator tapped it after the fix made confirm reachable again) — a read-only re-query immediately after found zero orphaned/duplicate data, but this does not substitute for the formal, deliberate Rollback validation the project's protocol requires, which remains the only item still open before Production Go. Both markers soft-deleted, zero residue confirmed; flags restored to `NO`; device fully reinstalled with the fixed, safe-flags build and confirmed to launch cleanly. Full regression: Node 864/864, `npm run smoke:sync` PASS, iOS Unit 584/584 (5 safe skips), GuestMergeTests 119/119, UI 6/6 (1 safe skip), Debug/Release clean builds 0 errors, `npm audit` clean, `git diff --check` clean. See `docs/INVENTORY_SYNC_PHYSICAL_DEVICE_RESULTS.md`'s "Phase 2B-8C" section. All flags remain `NO`; nothing pushed.

- Strengthened the Phase 2B-8C `resolveConflict` regression test: added explicit assertions that resolving the last conflict never auto-triggers an upload (transport applied-count and the local `PendingMutation` ledger both unchanged before/after), and that re-opening the merge flow afterward (a fresh `GuestMergeController` over the same persistence, simulating "close and reopen the sheet") lands back on `previewReady` with the choice intact — never regresses to a stuck conflict screen. Added a second, independent regression test, `testResolvingTheLastConflictWithKeepRemoteAlsoReturnsToPreviewReady`, proving the status-recovery fix generalizes across choices (it only ever checks `plan.conflicts.isEmpty`, never which action a candidate resolved to) rather than being coincidentally correct for `.skip` alone. GuestMergeTests now 120/120 (up from 119). Full regression re-confirmed: Node 864/864, `npm run smoke:sync` PASS, iOS Unit 585/585 (5 safe skips), UI 6/6 (1 safe skip), Debug/Release clean builds 0 errors, `npm audit` clean, `git diff --check` clean. All flags remain `NO`; nothing pushed.

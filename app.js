...
async function callCloudAI(pack, inv) {
  ...
  const prompt = `我冰箱里有：【${invNames}】。我的菜谱数据库有：【${recipeNames}】。
  请：1. 挑选 3 道最适合现在做的菜（消耗库存）。2. 推荐 1 道创意菜。
  只返回 JSON：{ "local": [ {"name": "菜名", "reason": "理由"} ], "creative": { "name": "菜名", "reason": "理由", "ingredients": "用料" } }`;
  try {
    const res = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${settings.apiKey}` }, body: JSON.stringify({ model, messages: [{role: "user", content: prompt}], temperature: 0.7 }) });
...
function renderRecipeDetail(id, pack) {
  ...
  div.innerHTML = `
    <div style="margin-bottom:20px;display:flex;justify-content:space-between;">
       ...
       <a class="btn" href="#recipe-edit:${r.id}">✎ 编辑 / 录入</a>
    </div>

...
const INGREDIENT_ALIASES = {
  ...
  "红苕": ["红薯", "地瓜", "甘薯", "红心红苕"],
  "莴笋": ["青笋", "莴苣", "莴笋头", "莴笋尖", "凤尾"],
  "蚕豆": ["胡豆", "鲜蚕豆", "扁豆", "蚕豆（扁豆）"], // 截图特例
  "豌豆": ["青豆", "鲜豌豆", "豌豆尖", "豆尖", "鲜豌豆仁"],

  // --- 菌菇 & 干货 ---
  "香菇": ["冬菇", "花菇", "干香菇", "水发香菇", "冬菇（香菇）"], // 截图特例
  "口蘑": ["干口蘑", "水发口蘑"],
  "木耳": ["黑木耳", "云耳", "水发木耳"],
  "黄花菜": ["兰花", "干黄花菜", "兰花（干黄花菜）", "金针菜"], // 截图特例
  "竹荪": ["水发竹荪", "干竹荪"],

  // --- 调料 & 其他 ---
  "面粉": ["中筋面粉", "白面", "面粉（面点）"], // 截图特例
  "花椒": ["红花椒", "青花椒", "花椒粒", "花椒面"],
...
function getCanonicalName(name) {
  ...
  // 1. 去除括号及内容 (解决 "兰花（干黄花菜）" 这种格式)
  // 很多时候括号里是解释，我们主要看括号前的，或者括号里的标准名
  // 策略：先看全名是否在字典里，如果不在，再尝试去掉括号
  ...
  // 3. 去除常见的形状后缀 (解决 "牛肉片"、"肥膘肉" 等)
  const suffixes = ["肉", "片", "丝", "末", "丁", "块", "条", "泥", "茸", "尖", "头", "仁", "皮"];
  // 尝试去掉1个或2个后缀 (如 "肉片")
  let cleanSuffix = cleanPrefix;
  for (const s of suffixes) {
     if (cleanSuffix.endsWith(s)) {
       const tryName = cleanSuffix.slice(0, -s.length);
       // 只有当去掉后缀后的词在我们的标准库里有定义时，才敢合并
       // 防止把 "鱼头" 变成 "鱼"，如果字典里明确区分了鱼头和鱼肉
...
function checkAlias(name) {
  if (INGREDIENT_ALIASES[name]) return name; // 是标准名
  for (const [canonical, aliases] of Object.entries(INGREDIENT_ALIASES)) {
    if (aliases.includes(name)) return canonical; // 是别名
  }
  return null;
}

...
1. 【材料存在与用量分离】ingredients 与 seasonings 的每个 item 必须是非空字符串。evidence 已确认的材料必须保留，不得因用量未知而省略整项。来源明确给出数量和单位时保留来源值；来源未提供用量时必须输出 qty="" 且 unit=""，不得猜测。只有来源明确写了“适量”或“少许”时才允许保留该表达。
...
JSON 字段如下：
{
  "name": "标准菜名",
  "tags": ["家常菜", "口味/菜系等标签"],
  "ingredients": [ {"item": "土豆", "qty": "2", "unit": "个"}, {"item": "青椒", "qty": "", "unit": ""} ],
  "seasonings":  [ {"item": "生抽", "qty": "2", "unit": "勺"}, {"item": "盐", "qty": "", "unit": "少许"}, {"item": "水", "qty": "", "unit": ""} ],
...
【用量与单位规则】
1. ingredients（核心食材）：
   - 来源明确给出数量和单位时，保留来源值，不做猜测换算。
   - 来源没有明确数量或单位时，qty 和 unit 都必须为空字符串；不要猜测，也不得删除该食材。
   - 示例：{"item":"五花肉","qty":"1","unit":"块"}、{"item":"青椒","qty":"","unit":""}。
2. seasonings（常备调料/介质）：
   - 来源明确给出用量时保留来源值；来源未给出时 qty 和 unit 都留空。
   - 仅来源明确写了“适量”或“少许”时，可将它保留在 unit 中并令 qty 留空。
   - 示例：{"item":"生抽","qty":"2","unit":"勺"}、{"item":"盐","qty":"","unit":"少许"}、{"item":"水","qty":"","unit":""}。
...
const MAX_FINAL_PROMPT_EVIDENCE_ITEMS_PER_CATEGORY = 128;

function buildBoundedFinalPromptEvidence(evidence) {
  if (!evidence || typeof evidence !== 'object' || Array.isArray(evidence)) return {};
  return {
    ...evidence,
    observedMainIngredients: Array.isArray(evidence.observedMainIngredients)
      ? evidence.observedMainIngredients.slice(0, MAX_FINAL_PROMPT_EVIDENCE_ITEMS_PER_CATEGORY)
      : [],
    observedSeasonings: Array.isArray(evidence.observedSeasonings)
      ? evidence.observedSeasonings.slice(0, MAX_FINAL_PROMPT_EVIDENCE_ITEMS_PER_CATEGORY)
      : []
  };
}
...
async function parseRecipeDraftWithAi({ text = '', imageBase64 = null, sourceType = 'manual', sourceMetadata = null } = {}) {
  ...
  const boundedFinalPromptEvidence = buildBoundedFinalPromptEvidence(evidence);
  const finalPromptPayload = JSON.stringify({ evidence: boundedFinalPromptEvidence, sourceDiagnostics: initialDiagnostics });

#!/usr/bin/env node
// Applies 《大众川菜》1979 Production Batch 3 from the frozen dry-run
...
const EXPECTED_PRODUCTION_IDS = ['dz1979-p212', 'dz1979-p216', 'dz1979-p218', 'dz1979-p221', 'dz1979-p206'];
...
if (JSON.stringify(actualIds) !== JSON.stringify(EXPECTED_PRODUCTION_IDS)) {
  console.error('Frozen dry-run selection does not match the expected Batch 3 five. Aborting.');
...
if (curated.recipes.length !== 136) {
  console.error(`Expected pre-promotion curated count 136, found ${curated.recipes.length}. Aborting.`);
  process.exit(1);
}

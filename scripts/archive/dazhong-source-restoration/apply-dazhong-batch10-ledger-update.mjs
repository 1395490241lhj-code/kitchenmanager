#!/usr/bin/env node
// Appends the Batch 10 entry to the production promotions ledger, keeping
// Batch 1-9's records untouched. Field shapes mirror the existing Batch 1-9
...
const EXPECTED_ENTRY_IDS = ['dz1979-p203', 'dz1979-p201', 'dz1979-p207'];

if (dryRun.verificationProblems.length > 0
  || JSON.stringify(dryRun.items.map((item) => item.entryId)) !== JSON.stringify(EXPECTED_ENTRY_IDS)) {
  console.error('Frozen Batch 10 dry-run is invalid or reordered. Aborting.');
  process.exit(1);
}

if (ledger.batches.some((batch) => batch.batchId === 'dz1979-production-b10')) {
  console.error('dz1979-production-b10 already exists in the ledger. Aborting.');
  process.exit(1);
}
const expectedBatchIds = [
  ...
  'dz1979-production-b09',
];
if (ledger.batches.length !== expectedBatchIds.length
  || ledger.batches.some((batch, i) => batch.batchId !== expectedBatchIds[i])) {
  console.error('Ledger does not contain exactly the expected Batch 1-9 entries. Aborting.');
  process.exit(1);
}

const BASELINE_COMMIT = '204646b66a0fe0ed804cac4611a30845e655e837';

const batch10Entries = dryRun.items.map((item) => ({
...
const nextLedger = {
  ...
  batches: [
    ...ledger.batches,
    {
      batchId: 'dz1979-production-b10',
      status: 'promoted',
      baselineCommit: BASELINE_COMMIT,
      dryRunArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch10-dry-run.v1.json',
      quantityReviewArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch10-quantity-review.v1.json',
      entries: batch10Entries,
    },
  ],
};
...
console.log(`Batch 10 entries: ${batch10Entries.map((e) => e.productionId).join(', ')}`);

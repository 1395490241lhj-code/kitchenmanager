#!/usr/bin/env node
// Applies 《大众川菜》1979 Production Batch 8 from the frozen dry-run
...
const EXPECTED_PRODUCTION_IDS = ['dz1979-p129', 'dz1979-p130'];
...
if (JSON.stringify(actualIds) !== JSON.stringify(EXPECTED_PRODUCTION_IDS)) {
  console.error('Frozen dry-run selection does not match the expected Batch 8 two. Aborting.');
...
if (curated.recipes.length !== 155) {
  console.error(`Expected pre-promotion curated count 155, found ${curated.recipes.length}. Aborting.`);
  process.exit(1);
}

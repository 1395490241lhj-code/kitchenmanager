#!/usr/bin/env node
// Applies 《大众川菜》1979 Production Batch 7 from the frozen dry-run
...
const EXPECTED_PRODUCTION_IDS = ['dz1979-p211', 'dz1979-p144'];
...
if (JSON.stringify(actualIds) !== JSON.stringify(EXPECTED_PRODUCTION_IDS)) {
  console.error('Frozen dry-run selection does not match the expected Batch 7 five. Aborting.');
...
if (curated.recipes.length !== 153) {
  console.error(`Expected pre-promotion curated count 153, found ${curated.recipes.length}. Aborting.`);
  process.exit(1);
}

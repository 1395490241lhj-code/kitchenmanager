#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 6 dry-run.
//
// Reuses the verified Batch 1 hard gates (frozen readiness audit) and the
// Batch 2/3/4/5 runtime gate: only core ingredients are checked against the
...
// -- Batch 6 runtime gate (core ingredients only, reused Batch 2 logic) ------
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch6-'));
  try {
...
const simulation = {
  ...
  auxiliaryGeneratedFiles: {
    recipeCurationRemoved: {
      ...
      note: `Batch 6 入选的 ${selectedCount} 道均有 method，curate 直接保留，不进入 removed；既有 removed 决定不变。`,
    },
    recipesNeedingCompletion: {
      changes: needingDiffers ? 'UNEXPECTED semantic change' : 'unchanged',
      note: 'Batch 6 不改变任何既有 needing 决定。',
    },
    recipeCurationSummaryMd: {
      changes: summaryDiffers ? `expected count changes from the ${selectedCount} additions` : 'unchanged',
      expectedLineChanges: [
        `原始菜谱（有效集）: 347 -> ${347 + selectedCount}`,
        `overlay 新增/补全净增: 83 -> ${83 + selectedCount}`,
        `curated 保留: 151 -> ${151 + selectedCount}`,
        `从有效集保留（有做法）: 130 -> ${130 + selectedCount}`,
        `从 overlay 补全 method: 130 -> ${130 + selectedCount}`,
        `从 overlay 补全 ingredients: 93 -> ${93 + selectedCount}`,
      ],
    },
  },
};
...
const iosDecodeAudit = {
  ...
  batch6Compatible: items.every((item) => (
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch6-dry-run.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  baseline: {
    main: '6474ef7e30db4b321faea33aace5152474c55709',
  ...
  selection: {
    ...
    note: '机械筛选，未硬编码结果；Batch 1/2/3/4/5 已 promotion 的 25 道不在候选池（promotionState=promoted）。',
  },
  items,
  quantityReviewPreview: {
    purpose: 'Batch 6 promotion 将引入的 curated qty/unit 的 source-restoration-reviewed 登记预览（与 Batch 1/2/3/4/5 quantity-review 同构）。真实 promotion 时另行生成正式 quantity-review artifact；本预览仅为 dry-run 证据。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch6-dry-run.v1.json',
);

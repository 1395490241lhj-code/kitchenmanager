#!/usr/bin/env node
// Appends the Batch 3 entry to the production promotions ledger, keeping
// Batch 1 and Batch 2's records untouched. Field shapes mirror the existing
// Batch 1/2 ledger entries exactly (entryId/productionId/name/sourceQuality/
...
if (ledger.batches.some((batch) => batch.batchId === 'dz1979-production-b03')) {
  console.error('dz1979-production-b03 already exists in the ledger. Aborting.');
  process.exit(1);
}
if (ledger.batches.length !== 2
  || ledger.batches[0].batchId !== 'dz1979-production-b01'
  || ledger.batches[1].batchId !== 'dz1979-production-b02') {
  console.error('Ledger does not contain exactly the expected Batch 1+2 entries. Aborting.');
  process.exit(1);
}

const BASELINE_COMMIT = '156315241822c3655fe6867d7d945a007e402b1c';

const batch3Entries = dryRun.items.map((item) => ({
...
const nextLedger = {
  ...
  batches: [
    ...ledger.batches,
    {
      batchId: 'dz1979-production-b03',
      status: 'promoted',
      baselineCommit: BASELINE_COMMIT,
      dryRunArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch3-dry-run.v1.json',
      quantityReviewArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch3-quantity-review.v1.json',
      entries: batch3Entries,
    },
  ],
};
...
console.log(`Batch 3 entries: ${batch3Entries.map((e) => e.productionId).join(', ')}`);

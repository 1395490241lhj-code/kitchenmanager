#!/usr/bin/env node
// Builds the Batch 10 quantity review artifact by mechanically extracting
// every qty/unit ingredient from the frozen Batch 10 dry-run's
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch10-quantity-review.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  purpose: 'Batch 10 dry-run proposed curated qty/unit 的 source-restoration-reviewed 登记。qty/unit 逐字段取自冻结 Batch 10 dry-run quantityReviewPreview.records，raw/normalized 数量取自 canonical source，evidenceType=source-restoration。不重新推算、不手改数值，不执行正式 promotion。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch10-quantity-review.v1.json',
);

#!/usr/bin/env node
// Builds the Batch 4 quantity review artifact by mechanically extracting
// every qty/unit ingredient from the frozen Batch 4 dry-run's
...
const problems = [];
if (records.length !== 43) problems.push(`record-count-not-43:${records.length}`);
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch4-quantity-review.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  purpose: 'Batch 4 promotion 引入的 curated qty/unit 的 source-restoration-reviewed 登记。qty/unit 逐字段取自冻结 Batch 4 dry-run quantityReviewPreview.records，raw/normalized 数量取自 canonical source，evidenceType=source-restoration。不重新推算、不手改数值。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch4-quantity-review.v1.json',
);

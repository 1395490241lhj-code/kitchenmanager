#!/usr/bin/env node
// Builds the Batch 3 quantity review artifact by mechanically extracting
// every qty/unit ingredient from the frozen Batch 3 dry-run's
...
const problems = [];
if (records.length !== 35) problems.push(`record-count-not-35:${records.length}`);
...
if (productionIdsInBatch.size !== 5) problems.push(`production-ids-not-5:${productionIdsInBatch.size}`);
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch3-quantity-review.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  purpose: 'Batch 3 promotion 引入的 curated qty/unit 的 source-restoration-reviewed 登记。qty/unit 逐字段取自冻结 Batch 3 dry-run quantityReviewPreview.records，raw/normalized 数量取自 canonical source，evidenceType=source-restoration。不重新推算、不手改数值。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch3-quantity-review.v1.json',
);

#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 10 dry-run.
//
// Reuses the verified Batch 1 hard gates (frozen readiness audit) and the
// Batch 2-9 runtime gate: only core ingredients are checked against the
...
// unmodified (never called for Batch 10 selection) purely so this file can
// assert, in its own regression tests, that legacy behavior is unchanged.
// Batch 1-9 frozen dry-run artifacts are never regenerated or touched by
// this file.
...
function passesHardGatesSameForEachRemediated(entryId, {
  skipMethodOnlyCheck = false,
  skipNonExactCheck = false,
} = {}) {
  const entry = readinessByEntryId.get(entryId);
  const recipe = recipeByEntryId.get(entryId);
  if (!entry || entry.promotionDisposition !== 'new-recipe-candidate') return false;
  ...
  const plan = entry.productionIngredientPlan;
  if (!skipNonExactCheck && plan.quantityReadiness !== 'exact-comparable') return false;
  if (!skipNonExactCheck && plan.inventoryIngredients.some((ing) => !ing.inventoryComparable)) return false;
  ...
  for (const ing of recipe.ingredients ?? []) {
    const quantity = ing.normalizedQuantity ?? {};
    const kind = quantity.kind;
    if (!skipNonExactCheck && !['exact-mass', 'exact-count'].includes(kind)) return false;
...
// This is the gate Batch 10 selection actually uses.
...
function passesHardGatesMethodOnlyNullRemediated(entryId, { skipNonExactCheck = false } = {}) {
  if (!passesHardGatesSameForEachRemediated(entryId, {
    skipMethodOnlyCheck: true,
    skipNonExactCheck,
  })) return false;
  const entry = readinessByEntryId.get(entryId);
  const plan = entry.productionIngredientPlan;
...
// -- Batch 10 remediation policy: allow-reviewed-nonexact-null -------------
// The allowlist is mechanically read from the committed final quantity
// review. Every allowed pair must be the only non-exact/non-comparable item
// for its entry, remain non-core under the live classifier, and project to
// qty=null/unit=null. Nothing else is relaxed.
const REVIEWED_NONEXACT_NULL_ALLOWLIST = new Map(
  Object.entries(finalQuantityReview.reviewedNonExactNullAllowlist ?? {})
    .map(([entryId, items]) => [entryId, new Set(items)]),
);

function passesHardGatesReviewedNonExactNull(entryId) {
  if (!passesHardGatesMethodOnlyNullRemediated(entryId, { skipNonExactCheck: true })) return false;
  const entry = readinessByEntryId.get(entryId);
  const recipe = recipeByEntryId.get(entryId);
  const allowed = REVIEWED_NONEXACT_NULL_ALLOWLIST.get(entryId) ?? new Set();

  for (const ingredient of recipe.ingredients ?? []) {
    const kind = ingredient.normalizedQuantity?.kind;
    if (['exact-mass', 'exact-count'].includes(kind)) continue;
    if (!allowed.has(ingredient.rawItemText)) return false;
  }
  for (const ingredient of entry.productionIngredientPlan.inventoryIngredients) {
    if (ingredient.inventoryComparable) continue;
    if (!allowed.has(ingredient.productionItem)) return false;
    if (ingredient.qty !== null || ingredient.unit !== null) return false;
    if (classifyIngredientCompatibility(ingredient.productionItem, null, null).role === 'core') return false;
  }
  return true;
}

const passesHardGates = passesHardGatesReviewedNonExactNull;

// -- Batch 10 runtime gate (core ingredients only, reused Batch 2 logic) ------
...
const items = selected.map((entryId) => {
  ...
  const reviewedNonExactAllowlist = REVIEWED_NONEXACT_NULL_ALLOWLIST.get(entryId);
  if (reviewedNonExactAllowlist) {
    transformNotes.push(
      `remediationPolicy=allow-reviewed-nonexact-null：${[...reviewedNonExactAllowlist].join('、')} 仅按 final quantity review 的精确 (entryId,item) allowlist 以 qty=null/unit=null 进入 production；保留 canonical/source provenance，不猜测数量、不全局放行非精确 quantity。`,
    );
  }
...
for (const item of items) {
  ...
  for (const productionIngredient of planIngredients) {
    ...
    quantityReviewRecords.push({
      ...
      dryRunArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch10-dry-run.v1.json',
      reviewStatus: 'approved',
    });
  }
}
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch10-'));
  try {
...
const simulation = {
  ...
  auxiliaryGeneratedFiles: {
    recipeCurationRemoved: {
      ...
      note: `Batch 10 入选的 ${selectedCount} 道均有 method，curate 直接保留，不进入 removed；既有 removed 决定不变。`,
    },
    recipesNeedingCompletion: {
      changes: needingDiffers ? 'UNEXPECTED semantic change' : 'unchanged',
      note: 'Batch 10 不改变任何既有 needing 决定。',
    },
    recipeCurationSummaryMd: {
      changes: summaryDiffers ? `expected count changes from the ${selectedCount} additions` : 'unchanged',
      expectedLineChanges: [
        `原始菜谱（有效集）: 355 -> ${355 + selectedCount}`,
        `overlay 新增/补全净增: 91 -> ${91 + selectedCount}`,
        `curated 保留: 159 -> ${159 + selectedCount}`,
        `从有效集保留（有做法）: 138 -> ${138 + selectedCount}`,
        `从 overlay 补全 method: 138 -> ${138 + selectedCount}`,
        `从 overlay 补全 ingredients: 101 -> ${101 + selectedCount}`,
      ],
    },
  },
};
...
const iosDecodeAudit = {
  ...
  batch10Compatible: items.every((item) => (
...
if (JSON.stringify(actualCheckpoint) !== JSON.stringify(expectedCheckpoint)) {
  problems.push(`batch10-checkpoint-mismatch:${JSON.stringify(actualCheckpoint)}`);
}
if (JSON.stringify([...selected].sort()) !== JSON.stringify(['dz1979-p201', 'dz1979-p203', 'dz1979-p207'])) {
  problems.push(`batch10-selected-mismatch:${selected.join(',')}`);
}
...
if (finalQuantityReview.safeToAllow !== true) {
  problems.push('final-quantity-review-not-safe');
}
if (finalQuantityReview.verificationProblems?.length > 0) {
  problems.push('final-quantity-review-has-verification-problems');
}
for (const [entryId, allowed] of REVIEWED_NONEXACT_NULL_ALLOWLIST) {
  for (const item of allowed) {
    const proposal = items.find((candidate) => candidate.entryId === entryId)
      ?.proposedOverlayIngredients?.[entryId]?.find((ingredient) => ingredient.item === item);
    if (!proposal || proposal.qty !== null || proposal.unit !== null) {
      problems.push(`reviewed-nonexact-not-null:${entryId}:${item}`);
    }
  }
}
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch10-dry-run.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  baseline: {
    main: '151de3b45d22ed328835ae2165564117fbcf8cb2',
  ...
  remediationPolicy: 'allow-safe-same-for-each+allow-reviewed-methodonly-null+allow-reviewed-nonexact-null',
  remediationPolicies: {
    inheritedFromBatch7: 'allow-safe-same-for-each',
    inheritedFromBatch8: 'allow-reviewed-methodonly-null',
    newThisRound: 'allow-reviewed-nonexact-null',
  },
  ...
  finalQuantityReviewArtifact: 'data/source-restoration/dazhong-chuancai-1979-final-quantity-blocker-review.v1.json',
  reviewedNonExactNullAllowlist: Object.fromEntries(
    [...REVIEWED_NONEXACT_NULL_ALLOWLIST.entries()].map(([id, set]) => [id, [...set]]),
  ),
  purpose: `《大众川菜》1979 Production Batch 10 reviewed non-exact remediation dry-run：以 final quantity review commit 151de3b45d22ed328835ae2165564117fbcf8cb2 为 baseline，冻结继承 Batch 7 safe same-for-each 与 Batch 8 reviewed methodOnly-null，只新增 final review 明确批准的 (entryId,item) 非精确 null/null allowlist。runtime gate 仅检查 core ingredients；不猜数量、不全局允许 approximate/range/non-exact，不实施 consumed-dual。本 generator 机械筛选剩余 candidate，并在临时目录用真实 curate-recipes.js 模拟；不写 production，不正式 promotion，不修改 Batch 1-9 frozen artifacts、runtime-name review 或 final quantity review。`,
  selection: {
    hardGateCriteria: [
      'promotionState=not-promoted',
      'promotionDisposition=new-recipe-candidate',
      'sourceQuality=ready-for-later-promotion-review',
      'productionQuantityReadiness=exact-comparable，或所有非精确项均精确命中 reviewedNonExactNullAllowlist',
      '所有不可比 production ingredient 必须命中 reviewedNonExactNullAllowlist 且 qty=null/unit=null、role!=core',
      ...
      'range/approximate/qualitative/unresolved 默认阻塞；仅 final review 逐条批准的 (entryId,item) 例外',
    ...
    note: '机械筛选，未硬编码候选；Batch 1-9 已 promotion 的 33 道不在候选池。继续冻结 Batch 7/8 两条窄规则；Batch 10 仅新增 final quantity review 中三组精确 (entryId,花椒) allowlist。',
  },
  items,
  quantityReviewPreview: {
    purpose: 'Batch 10 可能 promotion 的 curated qty/unit source-restoration-reviewed 登记预览（与 Batch 1-9 quantity-review 同构）。仅登记真实非 null qty/unit；本预览不执行正式 promotion。',
  ...
  iosDecodeAudit,
  writeTargets: [
    'data/source-restoration/dazhong-chuancai-1979-promotion-batch10-dry-run.v1.json',
    'data/source-restoration/dazhong-chuancai-1979-promotion-batch10-dry-run.v1.md',
  ],
...
const markdown = `# 《大众川菜》1979 Production Batch 10 dry-run

- baseline: ${output.baseline.main}
- policy: ${output.remediationPolicy}
- funnel: ${remainingCandidates.length} -> hard-blocked ${hardGateBlockedCount} -> after-hard ${hardGateSurvivors.length} -> runtime-blocked ${gateExclusions.runtimeNameGate.length} -> eligible ${eligible.length} -> selected ${selected.length}
- selected order: ${selected.join(', ')}
- temp curated: ${simulation.tempCurateResult.headCuratedCount} -> ${simulation.tempCurateResult.simulatedCuratedCount}
- quantity-review structured records: ${quantityReviewRecords.length}; reviewed non-exact null records: 0
- production/ledger/readiness: dry-run only, unchanged
- consumed-dual remaining: dz1979-p222, dz1979-p224, dz1979-p226
- verification problems: ${problems.length}
`;
fs.writeFileSync(
  path.join(repoRoot, 'data/source-restoration/dazhong-chuancai-1979-promotion-batch10-dry-run.v1.md'),
  markdown,
);

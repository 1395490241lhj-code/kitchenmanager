#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 3 dry-run.
//
...
// -- Batch 3 runtime gate (core ingredients only, reused Batch 2 logic) ------
...
for (const item of items) {
  ...
  for (const productionIngredient of planIngredients) {
    ...
    quantityReviewRecords.push({
      ...
      dryRunArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch3-dry-run.v1.json',
      reviewStatus: 'approved',
    });
  }
}
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch3-'));
  try {
...
const iosDecodeAudit = {
  ...
  batch3Compatible: items.every((item) => (
...
const output = {
  ...
  items,
  quantityReviewPreview: {
    purpose: 'Batch 3 promotion 将引入的 curated qty/unit 的 source-restoration-reviewed 登记预览（与 Batch 1/2 quantity-review 同构）。真实 promotion 时另行生成正式 quantity-review artifact；本预览仅为 dry-run 证据。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch3-dry-run.v1.json',
);

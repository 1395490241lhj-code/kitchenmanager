#!/usr/bin/env node
// Applies 《大众川菜》1979 Production Batch 2 from the frozen dry-run
...
const EXPECTED_PRODUCTION_IDS = ['dz1979-p187', 'dz1979-p202', 'dz1979-p205', 'dz1979-p188', 'dz1979-p196'];
...
if (JSON.stringify(actualIds) !== JSON.stringify(EXPECTED_PRODUCTION_IDS)) {
  console.error('Frozen dry-run selection does not match the expected Batch 2 five. Aborting.');
...
if (curated.recipes.length !== 131) {
  console.error(`Expected pre-promotion curated count 131, found ${curated.recipes.length}. Aborting.`);
  process.exit(1);
}

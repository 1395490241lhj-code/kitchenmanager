#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 1 dry-run.
//
// Mechanically selects the 5 safest new-recipe-candidates, produces the
// overlay -> curate -> provenance promotion preview, and simulates the real
// promotion chain against a temp copy (real scripts/curate-recipes.js) so
// the artifact records the exact expected production delta. No production
// file in the workspace is written.
...
const eligible = readiness.entries
  ...
  .filter((id) => readinessByEntryId.get(id).promotionDisposition === 'new-recipe-candidate')
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch1-'));
  try {
...
const iosDecodeAudit = {
  ...
  batch1Compatible: items.every((item) => (
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch1-dry-run.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  baseline: {
    main: '0ffd35d6b1ba15f066b7f9fa749f4402f362107d',
    applicationReady: false,
  },
  purpose: '《大众川菜》1979 Production Batch 1 dry-run：机械选出最安全的 5 道 new-recipe-candidate，给出 overlay -> curate -> provenance 的可复现 promotion 链预览，并在临时目录用真实 curate-recipes.js 模拟完整链路。不写任何 workspace production 文件，不做真实 promotion。',
  selection: {
    criteria: [
      ...
      '无 range/approximate/qualitative/unresolved',
    ...
    order: 'ingredient特殊结构最少 -> ingredient数量较少 -> method步骤较少 -> entryId升序',
    eligiblePoolCount: eligible.length,
    excludedSpecialRisks: {
      methodOnlyConversionWarning: ['dz1979-p129', 'dz1979-p130'],
      nonExactOrMixedQuantity: ['dz1979-p201', 'dz1979-p203', 'dz1979-p207'],
      consumedDualQuantity: ['dz1979-p222', 'dz1979-p224', 'dz1979-p226'],
      sameForEachCombinedQuantity: ['dz1979-p144', 'dz1979-p211'],
    },
    selectedEntryIds: selected,
    note: '本轮机械结果与上轮一致（p143/p204/p195/p200/p180），直接复用同一筛选规则与排序。',
  },
  items,
  promotionChain: [
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch1-dry-run.v1.json',
);

#!/usr/bin/env node
// Appends the Batch 5 entry to the production promotions ledger, keeping
// Batch 1/2/3/4's records untouched. Field shapes mirror the existing
...
if (ledger.batches.some((batch) => batch.batchId === 'dz1979-production-b05')) {
  console.error('dz1979-production-b05 already exists in the ledger. Aborting.');
  process.exit(1);
}
if (ledger.batches.length !== 4
  ...
  console.error('Ledger does not contain exactly the expected Batch 1+2+3+4 entries. Aborting.');
  process.exit(1);
}

const BASELINE_COMMIT = 'e2a92b1242e3efe84d8e2cb7a7c4664e8357e1a6';

const batch5Entries = dryRun.items.map((item) => ({
...
const nextLedger = {
  ...
  batches: [
    ...ledger.batches,
    {
      batchId: 'dz1979-production-b05',
      status: 'promoted',
      baselineCommit: BASELINE_COMMIT,
      dryRunArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch5-dry-run.v1.json',
      quantityReviewArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch5-quantity-review.v1.json',
      entries: batch5Entries,
    },
  ],
};
...
console.log(`Batch 5 entries: ${batch5Entries.map((e) => e.productionId).join(', ')}`);

#!/usr/bin/env node
// Appends the Batch 4 entry to the production promotions ledger, keeping
// Batch 1/2/3's records untouched. Field shapes mirror the existing
// Batch 1/2/3 ledger entries exactly (entryId/productionId/name/sourceQuality/
...
if (ledger.batches.some((batch) => batch.batchId === 'dz1979-production-b04')) {
  console.error('dz1979-production-b04 already exists in the ledger. Aborting.');
  process.exit(1);
}
if (ledger.batches.length !== 3
  ...
  console.error('Ledger does not contain exactly the expected Batch 1+2+3 entries. Aborting.');
  process.exit(1);
}

const BASELINE_COMMIT = 'a11c076a60f9c1568fc8a30fd6b7239d58cb0e9d';

const batch4Entries = dryRun.items.map((item) => ({
...
const nextLedger = {
  ...
  batches: [
    ...ledger.batches,
    {
      batchId: 'dz1979-production-b04',
      status: 'promoted',
      baselineCommit: BASELINE_COMMIT,
      dryRunArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch4-dry-run.v1.json',
      quantityReviewArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch4-quantity-review.v1.json',
      entries: batch4Entries,
    },
  ],
};
...
console.log(`Batch 4 entries: ${batch4Entries.map((e) => e.productionId).join(', ')}`);

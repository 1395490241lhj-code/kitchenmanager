#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 2 dry-run.
//
// Reuses the verified Batch 1 hard gates (frozen readiness audit) and adds
// the Batch 2 runtime gate: only core ingredients are checked against the
...
// -- Batch 2 runtime gate (core ingredients only) ---------------------------
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch2-'));
  try {
...
const iosDecodeAudit = {
  ...
  batch2Compatible: items.every((item) => (
...
const output = {
  ...
  selection: {
    ...
    note: '机械筛选，未硬编码结果；Batch 1 已 promotion 的 5 道不在候选池（promotionState=promoted）。',
  },
  items,
  quantityReviewPreview: {
    purpose: 'Batch 2 promotion 将引入的 curated qty/unit 的 source-restoration-reviewed 登记预览（与 Batch 1 quantity-review 同构）。真实 promotion 时另行生成正式 quantity-review artifact；本预览仅为 dry-run 证据。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch2-dry-run.v1.json',
);

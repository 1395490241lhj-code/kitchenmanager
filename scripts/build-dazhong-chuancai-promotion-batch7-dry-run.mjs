#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 7 dry-run.
//
// Reuses the verified Batch 1 hard gates (frozen readiness audit) and the
// Batch 2/3/4/5/6 runtime gate: only core ingredients are checked against the
...
// This is the exact legacy gate Batch 1-6 used. It is preserved here
// unmodified (never called for Batch 7 selection) purely so this file can
// assert, in its own regression tests, that legacy behavior is unchanged.
// Batch 1-6 frozen dry-run artifacts are never regenerated or touched by
// this file.
...
// -- Batch 7 remediation policy: allow-safe-same-for-each -------------------
//
...
//
//   legacy (Batch 1-6):   ANY ing.memberQuantityMode (same-for-each OR
...
// This is the gate Batch 7 selection actually uses.
const passesHardGates = passesHardGatesSameForEachRemediated;

// -- Batch 7 runtime gate (core ingredients only, reused Batch 2 logic) ------
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch7-'));
  try {
...
const simulation = {
  ...
  auxiliaryGeneratedFiles: {
    recipeCurationRemoved: {
      ...
      note: `Batch 7 入选的 ${selectedCount} 道均有 method，curate 直接保留，不进入 removed；既有 removed 决定不变。`,
    },
    recipesNeedingCompletion: {
      changes: needingDiffers ? 'UNEXPECTED semantic change' : 'unchanged',
      note: 'Batch 7 不改变任何既有 needing 决定。',
    },
    recipeCurationSummaryMd: {
      changes: summaryDiffers ? `expected count changes from the ${selectedCount} additions` : 'unchanged',
      expectedLineChanges: [
        `原始菜谱（有效集）: 349 -> ${349 + selectedCount}`,
        `overlay 新增/补全净增: 85 -> ${85 + selectedCount}`,
        `curated 保留: 153 -> ${153 + selectedCount}`,
        `从有效集保留（有做法）: 132 -> ${132 + selectedCount}`,
        `从 overlay 补全 method: 132 -> ${132 + selectedCount}`,
        `从 overlay 补全 ingredients: 95 -> ${95 + selectedCount}`,
      ],
    },
  },
};
...
const iosDecodeAudit = {
  ...
  batch7Compatible: items.every((item) => (
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch7-dry-run.v1',
  ...
  remediationPolicy: 'allow-safe-same-for-each',
  purpose: `《大众川菜》1979 Production Batch 7 mechanical remediation dry-run：复用 Batch 1 硬性 gate 的除 memberQuantityMode 判断外全部条件，与 Batch 2/3/4/5/6 已验证 runtime gate（仅 core ingredients，unresolved-name-match 阻塞、expected-unit-confirmation 记录不阻塞，禁止新增 alias/单位换算）。唯一规则变更：remediationPolicy=allow-safe-same-for-each，即 memberQuantityMode==="same-for-each" 不再 hard-block（unallocated-group-total 继续 block），因为其拆分逻辑已在 readiness productionIngredientPlan 中冻结验证（逐 member 精确继承同一 qty/unit，非比例分配/猜测）。机械选出最多 5 道剩余 new-recipe-candidate（本轮实际入选 ${selectedCount} 道，不足 5 不强行凑数），给出 overlay -> curate -> provenance 的可复现 promotion 链预览，并在临时目录用真实 curate-recipes.js 模拟完整链路。不写任何 workspace production 文件，不做真实 promotion，不修改 Batch 1-6 frozen dry-run artifacts。`,
  selection: {
    hardGateCriteria: [
    ...
    note: '机械筛选，未硬编码结果；Batch 1/2/3/4/5/6 已 promotion 的 27 道不在候选池（promotionState=promoted）。remediationPolicy=allow-safe-same-for-each：唯一放宽 memberQuantityMode==="same-for-each" 的 hard gate 判断，unallocated-group-total 继续 block，其余 legacy 条件不变。',
  },
  items,
  quantityReviewPreview: {
    purpose: 'Batch 7 promotion 将引入的 curated qty/unit 的 source-restoration-reviewed 登记预览（与 Batch 1/2/3/4/5/6 quantity-review 同构）。真实 promotion 时另行生成正式 quantity-review artifact；本预览仅为 dry-run 证据。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch7-dry-run.v1.json',
);

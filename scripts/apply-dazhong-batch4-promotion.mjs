#!/usr/bin/env node
// Applies 《大众川菜》1979 Production Batch 4 from the frozen dry-run
...
const EXPECTED_PRODUCTION_IDS = ['dz1979-p183', 'dz1979-p198', 'dz1979-p153', 'dz1979-p209', 'dz1979-p223'];
...
if (JSON.stringify(actualIds) !== JSON.stringify(EXPECTED_PRODUCTION_IDS)) {
  console.error('Frozen dry-run selection does not match the expected Batch 4 five. Aborting.');
...
if (curated.recipes.length !== 141) {
  console.error(`Expected pre-promotion curated count 141, found ${curated.recipes.length}. Aborting.`);
  process.exit(1);
}

#!/usr/bin/env node
// Builds the Batch 5 quantity review artifact by mechanically extracting
// every qty/unit ingredient from the frozen Batch 5 dry-run's
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch5-quantity-review.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  purpose: 'Batch 5 promotion 引入的 curated qty/unit 的 source-restoration-reviewed 登记。qty/unit 逐字段取自冻结 Batch 5 dry-run quantityReviewPreview.records，raw/normalized 数量取自 canonical source，evidenceType=source-restoration。不重新推算、不手改数值。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch5-quantity-review.v1.json',
);

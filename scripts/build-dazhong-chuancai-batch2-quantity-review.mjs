#!/usr/bin/env node
// Builds the Batch 2 quantity review artifact by mechanically extracting
// every qty/unit ingredient from the frozen Batch 2 dry-run's
...
const problems = [];
if (records.length !== 29) problems.push(`record-count-not-29:${records.length}`);
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch2-quantity-review.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  purpose: 'Batch 2 promotion 引入的 curated qty/unit 的 source-restoration-reviewed 登记。qty/unit 逐字段取自冻结 Batch 2 dry-run quantityReviewPreview.records，raw/normalized 数量取自 canonical source，evidenceType=source-restoration。不重新推算、不手改数值。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch2-quantity-review.v1.json',
);

#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 8 dry-run.
//
// Reuses the verified Batch 1 hard gates (frozen readiness audit) and the
// Batch 2/3/4/5/6/7 runtime gate: only core ingredients are checked against the
...
// unmodified (never called for Batch 8 selection) purely so this file can
// assert, in its own regression tests, that legacy behavior is unchanged.
// Batch 1-7 frozen dry-run artifacts are never regenerated or touched by
// this file.
...
// -- Batch 8 remediation policy: allow-safe-same-for-each -------------------
//
// remediationPolicy = 'allow-safe-same-for-each'
//
// This is the ONE mechanical rule change this round implements, exactly as
...
//   Batch 8 remediated:   only memberQuantityMode === 'unallocated-group-
...
// This is the gate Batch 8 selection actually uses.

// -- Batch 8 remediation policy: allow-reviewed-methodonly-null -------------
//
// remediationPolicy = 'allow-reviewed-methodonly-null'
//
// This is the SECOND (and only other) mechanical rule change this round
// implements, layered on top of Batch 7's same-for-each remediation, exactly
...
// -- Batch 8 runtime gate (core ingredients only, reused Batch 2 logic) ------
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch8-'));
  try {
...
const simulation = {
  ...
  auxiliaryGeneratedFiles: {
    recipeCurationRemoved: {
      ...
      note: `Batch 8 入选的 ${selectedCount} 道均有 method，curate 直接保留，不进入 removed；既有 removed 决定不变。`,
    },
    recipesNeedingCompletion: {
      changes: needingDiffers ? 'UNEXPECTED semantic change' : 'unchanged',
      note: 'Batch 8 不改变任何既有 needing 决定。',
    },
    recipeCurationSummaryMd: {
      changes: summaryDiffers ? `expected count changes from the ${selectedCount} additions` : 'unchanged',
      expectedLineChanges: [
        `原始菜谱（有效集）: 351 -> ${351 + selectedCount}`,
        `overlay 新增/补全净增: 87 -> ${87 + selectedCount}`,
        `curated 保留: 155 -> ${155 + selectedCount}`,
        `从有效集保留（有做法）: 134 -> ${134 + selectedCount}`,
        `从 overlay 补全 method: 134 -> ${134 + selectedCount}`,
        `从 overlay 补全 ingredients: 97 -> ${97 + selectedCount}`,
      ],
    },
  },
};
...
const iosDecodeAudit = {
  ...
  batch8Compatible: items.every((item) => (
...
const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch8-dry-run.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  baseline: {
    main: 'e9101fe990309f263c6cd1cdb249656d24ab6d61',
  ...
  remediationPolicies: {
    ...
    newThisRound: 'allow-reviewed-methodonly-null',
  },
  ...
  purpose: `《大众川菜》1979 Production Batch 8 mechanical remediation dry-run：复用 Batch 7 已验证 same-for-each remediated hard gate 与 Batch 2-7 已验证 runtime gate（仅 core ingredients，unresolved-name-match 阻塞、expected-unit-confirmation 记录不阻塞，禁止新增 alias/单位换算）。本轮唯一新增规则变更：remediationPolicy=allow-reviewed-methodonly-null，即仅 data/source-restoration/dazhong-chuancai-1979-methodonly-remediation-review.v1.json 中已逐条人工+扫描确认的 3 个 (entryId, item) 组合（p129:姜/花椒、p130:胡椒面）以 qty=null/unit=null 放行，其余任何 methodOnly core-no-quantity 条目继续 hard-block，不做全局自动放行。机械选出最多 5 道剩余 new-recipe-candidate（本轮实际入选 ${selectedCount} 道，不足 5 不强行凑数），给出 overlay -> curate -> provenance 的可复现 promotion 链预览，并在临时目录用真实 curate-recipes.js 模拟完整链路。不写任何 workspace production 文件，不做真实 promotion，不修改 Batch 1-7 frozen dry-run artifacts。`,
  selection: {
    hardGateCriteria: [
    ...
    note: '机械筛选，未硬编码结果；Batch 1-7 已 promotion 的 29 道不在候选池（promotionState=promoted）。remediationPolicy 复合两条：(1) allow-safe-same-for-each（Batch 7 已验证，same-for-each 放行、unallocated-group-total 继续 block）；(2) allow-reviewed-methodonly-null（本轮新增，仅 reviewedMethodOnlyNullAllowlist 中逐条确认的 3 个 (entryId, item) 组合放行 qty=null/unit=null，其余 methodOnly core-no-quantity 条目继续 hard-block）。其余 legacy 条件不变。',
  },
  items,
  quantityReviewPreview: {
    purpose: 'Batch 8 promotion 将引入的 curated qty/unit 的 source-restoration-reviewed 登记预览（与 Batch 1/2/3/4/5/6 quantity-review 同构）。真实 promotion 时另行生成正式 quantity-review artifact；本预览仅为 dry-run 证据。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch8-dry-run.v1.json',
);

#!/usr/bin/env node
// Builds the 《大众川菜》1979 Production Batch 5 dry-run.
//
// Reuses the verified Batch 1 hard gates (frozen readiness audit) and the
// Batch 2/3/4 runtime gate: only core ingredients are checked against the
...
// -- Batch 5 runtime gate (core ingredients only, reused Batch 2 logic) ------
...
for (const item of items) {
  ...
  for (const productionIngredient of planIngredients) {
    ...
    quantityReviewRecords.push({
      ...
      dryRunArtifact: 'data/source-restoration/dazhong-chuancai-1979-promotion-batch5-dry-run.v1.json',
      reviewStatus: 'approved',
    });
  }
}
...
function simulatePromotionChain() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'dz1979-batch5-'));
  try {
...
const simulation = {
  ...
  auxiliaryGeneratedFiles: {
    recipeCurationRemoved: {
      ...
      note: `Batch 5 入选的 ${selectedCount} 道均有 method，curate 直接保留，不进入 removed；既有 removed 决定不变。`,
    },
    recipesNeedingCompletion: {
      changes: needingDiffers ? 'UNEXPECTED semantic change' : 'unchanged',
      note: 'Batch 5 不改变任何既有 needing 决定。',
    },
    recipeCurationSummaryMd: {
      changes: summaryDiffers ? `expected count changes from the ${selectedCount} additions` : 'unchanged',
      expectedLineChanges: [
        `原始菜谱（有效集）: 342 -> ${342 + selectedCount}`,
        `overlay 新增/补全净增: 78 -> ${78 + selectedCount}`,
        `curated 保留: 146 -> ${146 + selectedCount}`,
        `从有效集保留（有做法）: 125 -> ${125 + selectedCount}`,
        `从 overlay 补全 method: 125 -> ${125 + selectedCount}`,
        `从 overlay 补全 ingredients: 88 -> ${88 + selectedCount}`,
      ],
    },
  },
};
...
const iosDecodeAudit = {
  ...
  batch5Compatible: items.every((item) => (
...
const output = {
  ...
  baseline: {
    main: 'af6a9e80a60610fb094bf55531f6532f512f27e2',
  ...
  purpose: `《大众川菜》1979 Production Batch 5 dry-run：复用 Batch 1 已验证硬性 gate 与 Batch 2/3/4 已验证 runtime gate（仅 core ingredients，unresolved-name-match 阻塞、expected-unit-confirmation 记录不阻塞，禁止新增 alias/单位换算），机械选出最多 5 道剩余 new-recipe-candidate（本轮实际入选 ${selectedCount} 道，不足 5 不强行凑数），给出 overlay -> curate -> provenance 的可复现 promotion 链预览，并在临时目录用真实 curate-recipes.js 模拟完整链路。不写任何 workspace production 文件，不做真实 promotion。`,
  selection: {
    hardGateCriteria: [
    ...
    note: '机械筛选，未硬编码结果；Batch 1/2/3/4 已 promotion 的 20 道不在候选池（promotionState=promoted）。',
  },
  items,
  quantityReviewPreview: {
    purpose: 'Batch 5 promotion 将引入的 curated qty/unit 的 source-restoration-reviewed 登记预览（与 Batch 1/2/3/4 quantity-review 同构）。真实 promotion 时另行生成正式 quantity-review artifact；本预览仅为 dry-run 证据。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch5-dry-run.v1.json',
);

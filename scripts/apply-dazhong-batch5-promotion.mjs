#!/usr/bin/env node
// Applies 《大众川菜》1979 Production Batch 5 from the frozen dry-run
...
const EXPECTED_PRODUCTION_IDS = ['dz1979-p162', 'dz1979-p186', 'dz1979-p185', 'dz1979-p219', 'dz1979-p213'];
...
if (JSON.stringify(actualIds) !== JSON.stringify(EXPECTED_PRODUCTION_IDS)) {
  console.error('Frozen dry-run selection does not match the expected Batch 5 five. Aborting.');
...
if (curated.recipes.length !== 146) {
  console.error(`Expected pre-promotion curated count 146, found ${curated.recipes.length}. Aborting.`);
  process.exit(1);
}

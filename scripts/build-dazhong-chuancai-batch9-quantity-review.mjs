#!/usr/bin/env node
// Builds the Batch 9 quantity review artifact by mechanically extracting
// every qty/unit ingredient from the frozen Batch 9 dry-run's
// quantityReviewPreview. The records are the source-restoration-backed
// reviewed quantities that would make the proposed curated ingredient qty/unit
...
if (records.length !== 18) problems.push(`batch9-record-count-mismatch:${records.length}`);
if (JSON.stringify(unitCounts) !== JSON.stringify({ g: 17, '只': 1 })) {
  problems.push(`batch9-unit-distribution-mismatch:${JSON.stringify(unitCounts)}`);
}

const output = {
  schema: 'kitchenmanager.source-restoration.promotion-batch9-quantity-review.v1',
  generatedAt: new Date().toISOString().slice(0, 10),
  purpose: 'Batch 9 dry-run proposed curated qty/unit 的 source-restoration-reviewed 登记。qty/unit 逐字段取自冻结 Batch 9 dry-run quantityReviewPreview.records，raw/normalized 数量取自 canonical source，evidenceType=source-restoration。不重新推算、不手改数值，不执行正式 promotion。',
...
const outPath = path.join(
  repoRoot,
  'data/source-restoration/dazhong-chuancai-1979-promotion-batch9-quantity-review.v1.json',
);

...
import { normalizeKitchenAmount, isSeasoning, getCanonicalName, UNIT_TYPE } from '../ingredients.js?v=205';
import { escapeOptionAttr, escapeHtml, setInlineStatus } from './status.js?v=205';
import { findInventoryMatch, formatInventoryAmount, getStockCoverageAnalysis, isIngredientMatch, getItemGear, GEAR_SCALE, GEAR_LABELS, gearInfo } from '../inventory.js?v=205';
import { loadShoppingItems, matchReceiptItemsToShoppingItems } from '../shopping.js?v=205';
...
export function showCalibrationModal(recipeName, predictions, onConfirm, onCancel) {
  ...
  panel.innerHTML = `
    <div class="km-modal-header calib-header">
      <span class="km-modal-title">✨ 烹饪消耗确认</span>
...
/**
 * 🛠️ 食材属性修正舱：液态玻璃弹窗，统一编辑「名称 / 存储环境 / 保质期 / 当前存量」。
 *  - PIECE（计件型）：当前存量用「− 数量 +」步进器。
 *  - GEAR（档位型）：当前存量用五档药丸（充足/大半/一半/见底/断货）。
 * @param {Object}   item              库存条目（就地修改）
 * @param {Object}   opts
 * @param {string}   opts.unitType     UNIT_TYPE.PIECE | UNIT_TYPE.GEAR
 * @param {Function} opts.onSave       保存后回调（持久化 + 重渲染）
 * @param {Function} [opts.onDelete]   删除该食材回调（可选）
 */
export function showIngredientEditModal(item, { unitType, onSave, onDelete } = {}) {
  const isPiece = unitType === UNIT_TYPE.PIECE;
  let qtyState = Math.max(0, Math.round(+item.qty || 0));
  let gearState = gearInfo(getItemGear(item)).value;
  let frozenState = !!item.isFrozen;

  const overlay = document.createElement('div');
  overlay.className = 'km-modal-overlay';
  const panel = document.createElement('div');
  panel.className = 'km-modal-content ingredient-edit-modal';

  const stockHtml = isPiece
    ? `<div class="cabin-stepper">
         <button type="button" class="calib-step" data-step="-1" aria-label="减少">−</button>
         <span class="cabin-qty-readout"><b class="cabin-qty">${qtyState}</b> ${escapeHtml(item.unit || '个')}</span>
         <button type="button" class="calib-step" data-step="1" aria-label="增加">+</button>
       </div>`
    : `<div class="calib-gear-pills cabin-gear-pills" role="group" aria-label="选择档位">${
        GEAR_SCALE.map(g => `<button type="button" class="calib-pill cabin-gear-pill${g === gearState ? ' is-active' : ''}" data-gear="${g}">${GEAR_LABELS[g]}</button>`).join('')
      }</div>`;

  panel.innerHTML = `
    <div class="km-modal-header">
      <span class="km-modal-title">🛠️ 修改食材：${escapeHtml(item.name)}</span>
      <button type="button" class="km-modal-close" aria-label="关闭">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
        </svg>
    ...
    <div class="km-modal-body cabin-body">
      <label class="pantry-manage-field cabin-field">
        <span>食材名称</span>
        <input type="text" class="km-modal-input" id="cabinName" value="${escapeOptionAttr(item.name)}" placeholder="食材名称">
      </label>
      <div class="pantry-manage-field cabin-field">
        <span>存储环境</span>
        <div class="cabin-seg" role="group" aria-label="存储环境">
          <button type="button" class="cabin-seg-btn${frozenState ? '' : ' is-active'}" data-frozen="0">🌡️ 常温/冷藏</button>
          <button type="button" class="cabin-seg-btn${frozenState ? ' is-active' : ''}" data-frozen="1">🧊 冷冻仓</button>
        </div>
      </div>
      <label class="pantry-manage-field cabin-field">
        <span>保质期（天）</span>
        <input type="number" min="0" step="1" class="km-modal-input" id="cabinShelf" value="${Number(item.shelf) || 7}" placeholder="保质期天数">
      </label>
      <div class="pantry-manage-field cabin-field">
        <span>当前存量</span>
        ${stockHtml}
      </div>
      <div id="cabinStatus" class="small inline-status" hidden></div>
      ${typeof onDelete === 'function' ? '<button type="button" class="cabin-delete-link" id="cabinDelete">🗑️ 删除该食材</button>' : ''}
    </div>
    <div class="km-modal-actions cabin-actions">
      <button type="button" class="btn ok cabin-save" id="cabinSave">保存修改</button>
    </div>
  `;
  ...
  // 存储环境分段切换
  panel.querySelectorAll('.cabin-seg-btn').forEach(btn => {
    btn.onclick = () => {
      frozenState = btn.dataset.frozen === '1';
      panel.querySelectorAll('.cabin-seg-btn').forEach(b => b.classList.toggle('is-active', b === btn));
    };
  });

  // 当前存量：计件步进 / 档位药丸
  if (isPiece) {
    const readout = panel.querySelector('.cabin-qty');
    panel.querySelectorAll('.calib-step').forEach(btn => {
      btn.onclick = () => {
        qtyState = Math.max(0, qtyState + (+btn.dataset.step));
        readout.textContent = qtyState;
      };
    });
  } else {
    panel.querySelectorAll('.cabin-gear-pill').forEach(pill => {
      pill.onclick = () => {
        gearState = +pill.dataset.gear;
        panel.querySelectorAll('.cabin-gear-pill').forEach(p => p.classList.toggle('is-active', p === pill));
      };
    });
  }

  if (typeof onDelete === 'function') {
    panel.querySelector('#cabinDelete').onclick = () => { close(); onDelete(); };
  }

  panel.querySelector('#cabinSave').onclick = () => {
    const statusEl = panel.querySelector('#cabinStatus');
    const name = panel.querySelector('#cabinName').value.trim();
    if (!name) { setInlineStatus(statusEl, '食材名称不能为空。', 'bad'); return; }
    const shelfVal = panel.querySelector('#cabinShelf').value.trim();
    const shelfNum = Number(shelfVal);
    if (shelfVal === '' || isNaN(shelfNum) || shelfNum < 0) {
      setInlineStatus(statusEl, '保质期必须是大于或等于 0 的数字。', 'bad');
      return;
    ...
    item.name = getCanonicalName(name) || name;
    item.isFrozen = frozenState;
    item.shelf = shelfNum;
    if (isPiece) {
      item.qty = qtyState;
      item.unitType = UNIT_TYPE.PIECE;
      item.stockStatus = qtyState <= 0 ? 'empty' : 'ok';
    } else {
      item.gear = gearState;
      item.unitType = UNIT_TYPE.GEAR;
      if (gearState === 0) { item.stockStatus = 'empty'; item.qty = 0; }
      else if (gearState <= 25) { item.stockStatus = 'low'; if (!(+item.qty > 0)) item.qty = 1; }
      else { item.stockStatus = 'ok'; if (!(+item.qty > 0)) item.qty = 1; }
    }
    close();

...
const RECEIPT_IMAGE_ENHANCED_ATTEMPTS = [
  { maxSide: 2200, quality: 0.92 },
  { maxSide: 2200, quality: 0.88 },
...
function normalizeReceiptItem(item) {
  ...
  if (typeof item === 'string') {
    ...
    rawName = String(item.rawName || item.rawText || item.originalText || '').trim();
    originalNameStr = String(item.originalName || '').trim() || rawName || nameStr;
    qty = item.qty ?? item.amount ?? 1;
    unitStr = String(item.unit || '').trim();
    reason = String(item.reason || '').trim();
    storage = String(item.storage || item.location || '').trim();
    uncertain = item.uncertain === true || String(item.confidence || '').toLowerCase() === 'low';
  }
...
function enhanceReceiptCanvas(ctx, width, height, mode = 'standard') {
  const strong = mode === 'enhanced';
  const contrast = strong ? 1.16 : 1.08;
  const brightness = strong ? 6 : 3;
  const grayscaleMix = strong ? 0.14 : 0.06;
  const sharpenAmount = strong ? 0.12 : 0.05;
  let imageData;
  try {
  ...
  const original = sharpenAmount > 0 ? new Uint8ClampedArray(data) : null;
  for (let i = 0; i < data.length; i += 4) {
    const gray = data[i] * 0.299 + data[i + 1] * 0.587 + data[i + 2] * 0.114;
    for (let c = 0; c < 3; c += 1) {
      const mixed = data[i + c] * (1 - grayscaleMix) + gray * grayscaleMix;
      data[i + c] = Math.max(0, Math.min(255, (mixed - 128) * contrast + 128 + brightness));
    }
  }
  if (original && width * height <= 5200000) {
    for (let y = 1; y < height - 1; y += 1) {
      for (let x = 1; x < width - 1; x += 1) {
        ...
        for (let c = 0; c < 3; c += 1) {
          ...
          data[idx + c] = Math.max(0, Math.min(255, current + (current - blur) * sharpenAmount));
        }
      }
    }
  }
...
function compressImage(file, options = {}) {
  return new Promise((resolve, reject) => {
    const mode = options.enhanced ? 'enhanced' : 'standard';
    const attempts = options.enhanced ? RECEIPT_IMAGE_ENHANCED_ATTEMPTS : RECEIPT_IMAGE_COMPRESSION_ATTEMPTS;
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        for (const attempt of attempts) {
          ...
          enhanceReceiptCanvas(ctx, w, h, mode);
...
export async function recognizeReceipt(file, options = {}) {
  const base64 = await compressImage(file, { enhanced: Boolean(options.enhanced) });
...
字段及要求：
- 只提取可以作为厨房库存管理的商品。请忽略店名、日期、地址、税费、折扣、优惠、会员、积分、总价、小计、付款方式、卡号、收银信息、发票号、条形码、袋费、容器费。
- 请把超市商品名归一化成家庭厨房常用食材名，例如：
  - "A2 MILK 3.25%" / "HOMO MILK" -> "牛奶"
  - "EGGS 12CT" / "LARGE EGGS" -> "鸡蛋"
  - "BEEF" / "SLICED BEEF" -> "牛肉"
  - "PORK BELLY" -> "五花肉"
  - "CHICKEN LEG" / "CHICKEN THIGH" -> "鸡腿"
  - "TOFU" / "MEDIUM FIRM TOFU" -> "豆腐"
- 不要把具体蔬菜、菇类强行泛化成“青菜”或“蘑菇”。能识别具体名称时必须保留具体名称：
  - 上海青、小白菜、油麦菜、菜心、芥兰、茼蒿、空心菜、大白菜、娃娃菜、生菜、菠菜、韭菜、豆苗
  - 皇子菇、金针菇、杏鲍菇、香菇、平菇、蟹味菇、白玉菇
- 英文或拼音商品名请尽量映射为具体名称：
  - "shanghai bok choy" -> "上海青"
  - "bok choy" / "pak choi" -> "小白菜"；如果无法确认品种，设置 uncertain: true
  - "yu choy" / "choy sum" -> "菜心"
  - "gai lan" / "kailan" -> "芥兰"
...
- 同一种食材重复出现时请尽量合并，数量相加或保留为合理包装数量。
- 不确定但像食物的商品放入 review，并设置 "uncertain": true；确定项设置 "uncertain": false。
- 每个可识别商品可以额外返回 storage 字段：冷藏 | 冷冻 | 常温 | 常备。生鲜肉蛋奶蔬菜通常冷藏，速冻品冷冻，干货调料常备。
- 可额外返回 rawName 字段，等同于小票商品名；rawName/rawText 用于调试，不会直接展示给用户。
...
- name: 必须是常见的中厨房常见中文食材名称。不要做生硬的字面直译。
  ...
  - 必须参考以下常见英文名与中文食材名的映射范例：
    ...
    - "bok choy" -> "小白菜"（不确定时 uncertain: true，不要写成青菜）
    - "baby bok choy" -> "小白菜"
    - "yu choy" / "choy sum" -> "菜心"
    - "gai lan" / "kailan" -> "芥兰"
    - "tong ho" -> "茼蒿"
...
- 具体蔬菜、莴笋、豆芽、yu choy、stem lettuce、beansprout、鸡腿、pork、beef、shrimp、fish 等鲜货放入 inventory；不要把 yu choy / tong ho / bok choy 统一写成青菜。

...
- Home: ordinary cooking/flexible days show one tappable date row; exceptional state may add one meaningful context line. No redundant hero `今晚`. Presence-only readiness uses `所需食材已在库` / `N/M 食材已在库` over all today's plans. One dish is hero-only; two add `配 …`; three or more use `另有 N 道` with only remaining dishes inside. No aggregate multi-dish duration. At most two named attention rows plus overflow. `开始做饭` enters the shared cooking flow; `查看菜谱` remains secondary beside it. Planner owns planning management as a first-class bottom tab; Home has no planning-management row. Recommendation discovery is inline (D-048): in decision mode `今天怎么吃` owns a view-aligned horizontal shelf with the next card visibly peeking; Accessibility sizes stack the same cards vertically. In execution mode a quieter `更多推荐` shelf follows `需要处理`, with non-dominant actions so the current cooking task keeps the only prominent CTA. `换一批` is the shelf's terminal action; Home no longer requires a `更多推荐` navigation gate merely to inspect another dish. `今天的计划` remains absent.
...
- Profile / Settings: intentionally retains native SwiftUI `Form` presentation. Native section headers, Pickers, Toggles, NavigationLinks and system confirmations provide the structure; account status leads without becoming a hero or warning. Trust and data explanations remain complete but secondary. Decorative icons may disappear at Accessibility sizes to preserve readable text width. AI Settings remains an ordinary settings destination, not a separate visual brand; the destructive local-data action stays isolated at the end. No custom Quiet Kitchen dashboard or card conversion is currently justified.
...
### Home hierarchy and tactile surfaces — accepted 2026-09-25
...
semantics and presentation until deliberately brought into this language. Profile / Settings was the next adoption
boundary in this earlier direction; the current production revision above supersedes that direction and preserves its native Form. No recipe media dependency.
